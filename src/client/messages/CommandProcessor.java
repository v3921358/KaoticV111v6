...
public class CommandProcessor {
    ...
    static {
        Class<?>[] CommandFiles = {
            ...
            PlayerCommand.class, DonatorCommand.class, GMCommand.class, AdminCommand.class
        };
    ...
    public static boolean processCommand(MapleClient c, String line, CommandType type) {
        ...
        if (curr - c.getPlayer().getCommandCoolDown() >= cooldown) {
            if (c.getPlayer().getGMLevel() > PlayerGMRank.NORMAL.getLevel()) {
                if (line.charAt(0) == PlayerGMRank.GM.getCommandPrefix() || line.charAt(0) == PlayerGMRank.ADMIN.getCommandPrefix()) { //Redundant for now, but in case we change symbols later. This will become extensible.
                    String[] splitted = line.split(" ");
                    splitted[0] = splitted[0].toLowerCase();
    ...
    private static void logCommandToDB(MapleCharacter player, String command, String table) {
        ...
        try {
            ps = DatabaseConnection.getPlayerConnection().prepareStatement("INSERT INTO " + table + " (cid, command, mapid) VALUES (?, ?, ?)");
