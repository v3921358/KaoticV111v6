...
public class MonsterBook implements Serializable {
    ...
    public final static MonsterBook loadCards(final int charid, final MapleCharacter chr) {
    ...
    public final boolean monsterCaught(final MapleClient c, final int cardid, final String cardname) {
        if (!cards.containsKey(cardid) || cards.get(cardid) < 2) {
            ...
            c.announce(EffectPacket.showForeignEffect(16));
            cards.put(cardid, 2);
            if (GameConstants.GMS) {
                ...
                if (rr > 0) {
                    ...
                    c.announce(EffectPacket.showForeignEffect(43));
                    if (rr > 1) {
