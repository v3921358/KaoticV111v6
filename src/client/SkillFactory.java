...
public class SkillFactory {
    ...
    private static final Map<Integer, List<Integer>> skillsByClass = new HashMap<Integer, List<Integer>>();
    ...
    public static void load() {
                    ...
                    if (data.getName().equals("skill")) {
                        ...
                        for (MapleData data2 : data) { // Loop thru each jobs
                            if (data2 != null) {
                                ...
                                job.add(skillid);
                                List<Integer> skillClass = skillsByClass.get(skillid / 100000);
                                if (skillClass == null) {
                                    skillClass = new ArrayList<Integer>();
                                    skillsByClass.put(skillid / 100000, skillClass);
                                }
                                skillClass.add(skillid);
        ...
        int count = 0;
        try (Connection con = DatabaseConnection.getWorldConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM skill_template"); ResultSet rs = ps.executeQuery();) {
            while (rs.next()) {
                GameConstants.addAttackSkill(rs.getInt("skillid"), rs.getInt("attacks"), rs.getInt("lines"));
                count++;
            }
        ...
        System.out.println(count + " Custom Skills Loaded");
        //saveAch();
    ...
    public static List<Integer> getSkillsByClass(final int jobId) {
        return skillsByClass.get(jobId);
    }
