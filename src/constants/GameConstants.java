...
public class GameConstants {
    ...
    public static int STARTING_MAP = 5000;
    public static int OVERLEVEL = 250;
    public static double EXPRATE = 1.0;
    public static int VOTES = 0;

    public static boolean GMS = true; //true = GMS
    public static boolean globalEvent = false; //true = GMS
    public static boolean capped = true; //toggle - server caps
    public static int PartySize = 6; //true = GMS
    public static int serverKDBonus = 0;
    ...
    private static final int[] pvpExp = {0, 3000, 6000, 12000, 24000, 48000, 96000, 192000, 384000, 768000};
    private static int[] guildexp = {0, 20000, 160000, 540000, 1280000, 2500000, 4320000, 6860000, 10240000, 14580000, 14643246, 14677117, 14723663, 14785956, 14867479, 14972140, 15104288, 15268732, 15470751, 15716112, 16011084, 16362448, 16777516, 17264141, 17830728, 18486250, 19240259, 20102897, 21084908, 22197649, 23453105, 24863895, 26443283, 28205195, 30164222, 32335634, 34735392, 37380153, 40287283, 43474869, 46961723, 50767397, 54912190, 59417155, 64304112, 69595658, 75315168, 81486815, 88135570, 95287214, 102968348, 111206397, 120029626, 129467138, 139548891, 150305703, 161769258, 173972120, 186947732, 200730432, 215355457, 230858950, 247277972, 264650503, 283015456, 302412679, 322882968, 344468068, 367210687, 391154496, 416344145, 442825260, 470644460, 499849355, 530488561, 562611701, 596269415, 631513366, 668396246, 706971785, 747294755, 789420978, 833407333, 879311762, 927193277, 977111966, 1029128998, 1083306633, 1139708227, 1198398236, 1259442224, 1322906871, 1388859976, 1457370467, 1528508403, 1602344982, 1678952549, 1758404600, 1840775786, 2000000000};
    ...
    public static final String[] stats = {"tuc", "reqLevel", "reqJob", "reqSTR", "reqDEX", "reqINT", "reqLUK", "reqPOP", "cash", "cursed", "success", "setItemID", "equipTradeBlock", "durability", "randOption", "randStat", "masterLevel", "reqSkillLevel", "elemDefault", "incRMAS", "incRMAF", "incRMAI", "incRMAL", "canLevel", "skill", "charmEXP", "android", "epicItem"};
    ...
    public static Map<Integer, Pair<Integer, Integer>> skill_attacks = new ConcurrentHashMap<Integer, Pair<Integer, Integer>>();
    public static Map<Integer, Pair<Integer, Integer>> skill_attacks_random = new ConcurrentHashMap<Integer, Pair<Integer, Integer>>();
    public static boolean kaotic = false;
    public static boolean lockSlot = false;
    public static Map<String, Long> serverVar = new ConcurrentHashMap<String, Long>();
    public static int stat_rate = 1, donation_rate = 0, stat_rate_base = 10;
    public static double mastery_rate = 1.0;
    public static List<Integer> npcs = new ArrayList<>();
    public static List<Integer> pals = new ArrayList<>();
    private static final List<Integer> blockNpc = Arrays.asList(new Integer[]{1500, 1501, 1502, 1503, 1504, 1505, 1506, 9010022, 1540809, 9401088, 9071003, 9000000, 9010106});
    public static boolean shutdown = false;

    public static void setMasteryRate(double value) {
        mastery_rate = Randomizer.DoubleMin(value, 1.0);
    }

    public static double getMasteryRate() {
        return stat_rate;
    }

    public static void setStatRate(int value) {
        stat_rate = value;
    }

    public static void setBaseStatRate(int value) {
        stat_rate_base = value;
    }

    public static int getStatRate() {
        return stat_rate;
    }

    public static int getbaseStatRate() {
        return stat_rate_base;
    }

    public static boolean isBlockedNpc(int id) {
        return blockNpc.contains(id);
    }

    public static void setDonationRate(int value) {
        donation_rate = value;
    }

    public static int getDonationRate() {
        return donation_rate;
    }
    ...
    public static void addNpc(MapleCharacter chr, int id) {
        if (!npcs.contains(id)) {
            npcs.add(id);
            try (Connection con = DatabaseConnection.getWorldConnection()) {
                PreparedStatement ps = con.prepareStatement("INSERT INTO pal_trainer (id, charid, level, min_level, max_level, min_pal, max_pal, tier, rank, iv, cooldown, bg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
                ps.setInt(1, id);
                ps.setInt(2, chr.getAccountID());
                int level = Randomizer.random(1, 100);
                ps.setInt(3, level);
                ps.setInt(4, level);
                ps.setInt(5, Randomizer.Max(Randomizer.random(level, (int) Math.floor(level * 1.25)), 100));
                int pals = Randomizer.random(1, 3);
                ps.setInt(6, pals);
                ps.setInt(7, Randomizer.random(pals, 6));
                ps.setInt(8, 1);
                ps.setInt(9, 1);
                ps.setInt(10, Randomizer.random(1, 250));
                ps.setInt(11, 3600);
                ps.setInt(12, Randomizer.random(1, 36));
                ps.execute();
                ps.close();
            } catch (SQLException e) {
                e.printStackTrace();
            }
            //chr.dropMessage(1, "You have claimed this npc!");
            //chr.gainItem(4310510, 1);
        } else {
            //chr.dropMessage(1, "Npc has already been claimed.");
        }
    }

    public static void loadNpcs() {
        int count = 0;
        try (Connection con = DatabaseConnection.getWorldConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM pal_trainer"); ResultSet rs = ps.executeQuery();) {
            while (rs.next()) {
                npcs.add(rs.getInt("id"));
                count++;
            }
        } catch (SQLException ess) {
            ess.printStackTrace();
            System.out.println("Failed to load npcs");
            FileoutputUtil.outputFileError(FileoutputUtil.PacketEx_Log, ess);
        }
        System.out.println("Loaded " + count + " Pal Trainers");
    }
    ...
    public static void setServerVar(String name, long value) {
        if (!serverVar.containsKey(name)) {
            serverVar.put(name, (long) 0);
        }
        serverVar.put(name, value);
    }

    public static void addServerVar(String name, long value) {
        if (!serverVar.containsKey(name)) {
            serverVar.put(name, (long) 0);
        }
        serverVar.put(name, serverVar.get(name) + value);
    }

    public static long getServerVar(String name) {
        if (serverVar.get(name) != null) {
            return serverVar.get(name);
        } else {
            return 0;
        }
    }
    ...
    public static String getMeta() {
        ...
        switch (month) {
            case 1:
                return "[Meta January] Fishing 200% buff";
            case 2:
                return "[Meta Febuary] Damage Skin 300% buff";
            case 3:
                return "[Meta March] Maple Pal's effects 500% buff";
            case 4:
                return "[Meta April] Mastery Effects 300% buff";
            case 5:
                return "[Meta May] Dungeon Rewards 200% buff";
            case 6:
                return "[Meta June] Damage Skin 250% buff";
            case 7:
                return "[Meta July] Damage 200% buff";
            case 8:
                return "[Meta August] Damage 250% buff";
            case 9:
                return "[Meta September] Mastery Effects 500% buff";
            case 10:
                return "[Meta October] Use-Etc Drops 250% buff";
            case 11:
                return "[Meta November] Damage Skin 500% buff";
            case 12:
                return "[Meta December] Maple Pal's 1000% effects buff";
        }
        return "[Meta] no meta effects active this month.";
    }

    public double metaRate(int cMonth, double amount) {
        int month = Calendar.getInstance().get(Calendar.MONTH);
        if (cMonth == month) {
            return amount;
        } else {
    ...
    public static void setCapped(boolean toggle) {
        capped = toggle;
    }

    public static int getBonusKD() {
        return serverKDBonus;
    }

    public static void setBonusKD(int value) {
        serverKDBonus = value;
    }

    public static boolean getCapped() {
        return capped;
    }

    public static int getMaxHpMp() {
        return 9999999;
    }

    public static boolean isMegaBoss(int id) {
        switch (id) {
            case 9390600:
            case 9390601:
            case 9390602:
    ...
    public static int getMaxMp(int job) {
        switch (job) {
            case 3001:
                return 10;
            case 3100:
    ...
    public static int getGuildMaxLevel() {
        return guildexp.length;
    }

    public static void setLockSlot(boolean toggle) {
        lockSlot = toggle;
    }

    public static boolean getLockSlot() {
        return lockSlot;
    }

    public static void setGlobalEvent(boolean toggle) {
        globalEvent = toggle;
    }

    public static boolean getGloablEvent() {
        return globalEvent;
    }

    public static void addAttackSkill(int id, int attacks, int lines) {
        if (!skill_attacks.containsKey(id)) {
            skill_attacks.put(id, new Pair<>(attacks, lines));
        }
    }

    public static Pair<Integer, Integer> getAttackSkill(int id) {
        return skill_attacks.get(id);
    }

    public static void clearRandomAttacks() {
        if (!skill_attacks_random.isEmpty()) {
            skill_attacks_random.clear();
        }
    }
    ...
    public static int getStartingMap() {
        return STARTING_MAP;
    }
    ...
    public static double getExpRate() {
        return EXPRATE;
    }

    public static void setVotes(int value) {
        VOTES = value;
    }
    ...
    public static int getJobBranch(short job) {
        if (job % 1000 == 0) {
            return 0;
        } else if (job % 100 == 0) {
            return 1;
    ...
    public static int getJobMaxLevel(short job) {
    ...
    public static boolean isKannaMage(final int job) {
        return (job >= 1200 && job <= 1212);
    }

    public static boolean isPath(final int job) {
        return (job >= 1300 && job <= 1312);
    }

    public static boolean isNW(final int job) {
        return (job >= 1400 && job <= 1412);
    }

    public static boolean isArk(final int job) {
    ...
    public static boolean isKain(final int job) {
        return job >= 1100 && job <= 1112;
    }

    public static int getSubJobs(int job) {
        if (isEvan(job)) {
            return 10;
        }
        return 4;
    ...
    public static boolean isThrowingStar(final int itemId) {
        return (int) Math.floor(itemId / 10000) == 207;
    }

    public static boolean isBullet(final int itemId) {
        return (int) Math.floor(itemId / 10000) == 233;
    }
    ...
    public static boolean isOverall(final int itemId) {
        return (int) Math.floor(itemId / 10000) == 105;
    }

    public static boolean isDoubleSlot(final int itemId) {
        if (isOverall(itemId) && !MapleItemInformationProvider.getInstance().isCash(itemId)) {
            return true;
        }
        int cat = (int) Math.floor(itemId / 10000);
        cat = cat % 100;
        switch (cat) { // 39, 50, 51 ??
            case 40:
            case 41:
    ...
    public static boolean isWeapon(final int itemId) {
        final int s = (int) Math.floor(itemId / 10000.0);
        switch (s) {
            case 130, 131, 132, 133, 136, 137, 138, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 152, 153, 170 -> {
                return true;
            }
        }
        return false;
    ...
    public static boolean isOverFlowable(int itemid) {
        return getInventoryType(itemid) == MapleInventoryType.ETC || getInventoryType(itemid) == MapleInventoryType.USE;
    }
    ...
    public static boolean canOverflowItem(final int itemId) {
        final byte type = (byte) (itemId / 1000000);
        if (type == 2 || type == 4) {
            return true;
        }
    ...
    public static int getInventoryTypeId(final int itemId) {
        final byte type = (byte) (itemId / 1000000);
        if (type < 1 || type > 5) {
            return 0;
        }
        return MapleInventoryType.getByType(type).getType();
    }
    ...
    public static boolean isChaosScroll(int itemId) {
        if (itemId == 2049116 || itemId == 2049117 || itemId == 2049118) {
            return true;
        }
        if (itemId >= 2049180 && itemId <= 2049184) {
            return false;
        }
        if (itemId >= 2049190 && itemId <= 2049199) {
            return false;
        }
    ...
    public static int getChaosNumber(int itemId) {
        switch (itemId) {
            case 2049177://inf               
                return 250;
            case 2049176://supreme               
                return 100;
            case 2049175://extreme               
                return 50;
            case 2049189://Genesis               
                return 25;
            default:
                return 1;
        }
    ...
    public static int getEENumber(int itemId) {
        return itemId == 2049300 ? 2 : 1;
    }

    public static int getEEChance(int itemId) {
        if (itemId / 100 == 20493) {
            return 10;
        }
        return 100;
    ...
    public static boolean isPotentialScroll(int scrollId) {
        return scrollId / 100 == 20494 || scrollId / 100 == 20497 || scrollId / 1000 == 2043 || scrollId == 5534000;
    }

    public static boolean isEnhancedScroll(final int scrollId) {
        switch (scrollId) {
            case 2049300: // adv
            case 2049301: // normal
                return true;
    ...
    public static boolean isRealTwoHanded(final int itemId) {
        switch (getWeaponType(itemId)) {
            case AXE2H:
            case GUN:
            case KNUCKLE:
    ...
    public static int[] blockedMaps = {180000001, 180000002, 109050000, 280030000, 240060200, 280090000, 280030001, 240060201, 950101100, 950101010, 5000, 5001, 5002, 5003, 5004, 5005, 10};
    ...
    public static boolean potentialIDFits(final int potentialID, final int newstate) {
        //first line is always the best
        //but, sometimes it is possible to get second/third line as well
        //may seem like big chance, but it's not as it grabs random potential ID anyway
        if (newstate == 20) {
            return (Randomizer.nextInt(10) == 0 ? potentialID >= 40000 : potentialID >= 30000 && potentialID < 60004); // xml say so
        } else if (newstate == 19) {
            return (Randomizer.nextInt(10) == 0 ? potentialID >= 30000 : potentialID >= 20000 && potentialID < 30000);
        } else if (newstate == 18) {
            return (Randomizer.nextInt(10) == 0 ? potentialID >= 20000 && potentialID < 30000 : potentialID >= 10000 && potentialID < 20000);
        } else if (newstate == 17) {
            return (Randomizer.nextInt(10) == 0 ? potentialID >= 10000 && potentialID < 20000 : potentialID < 10000);
        } else {
            return false;
        }
    }
    ...
    public static boolean isCKJob(final int job) {
        return job > 1000 && job < 1600;
    }
    ...
    public static boolean isWarriorJob(final int job) {
        return job >= 100 && job <= 132 || isDemon(job) || isKain(job) || isAran(job);
    }

    public static boolean isMageJob(final int job) {
        return job >= 200 && job <= 232 || isEvan(job) || isBattleMage(job) || isKannaMage(job);
    }

    public static boolean isBowmanJob(final int job) {
        return job >= 300 && job <= 332 || isMercedes(job) || isWH(job) || isPath(job);
    }

    public static boolean isThiefJob(final int job) {
        return job >= 400 && job <= 434 || isNW(job);
    }

    public static boolean isPirateJob(final int job) {
        return job >= 500 && job <= 532 || isMech(job) || isArk(job);
    }

    public static boolean isGMJob(final int job) {
        return job == 900 || job == 910;
    }

    public static String getClassesCombination(int value) {
        Map<Integer, String> classes = new HashMap<>();
        classes.put(1, "Warrior");
        classes.put(2, "Mage");
        classes.put(4, "Bow");
        classes.put(8, "Thief");
        classes.put(16, "Pirate");
    ...
    public static String convertText(String text) {
        String formattedComment = text.replaceAll("\\P{Print}", "");
    ...
    public static boolean isForceShield(int id) {
        return id >= 1099000 && id <= 1099015;
    }

    public static boolean isAndroid(int id) {
        return id >= 1662000 && id < 1663000;
    }

    public static boolean isAndroidHeart(int id) {
        return id >= 1663000 && id < 1664000;
    }
    ...
    public static boolean isQuestItem(int id) {
        return id / 10000 >= 400 && id / 10000 <= 403;
    }
    ...
    public static int getMPByJob(int job) {
        ...
        return 10; // beginner or 3100
    }
    //keep npcs: mobdrops - 9000085 : 9270035 - uni npc

    //public static final int[] publicNpcIds = {9270035, 9010022, 9071003, 9000087, 9000088, 9010000, 9000085, 9000018, 9000000};
    //order of npcs, (warp, job, recycle, mobinfo, monsterpark)
    public static final int[] publicNpcIds = {9010022, 9010038, 9000000, 9010000, 9000085};
    public static final String[] publicNpcsScripts = {"", ""};
    public static final String[] publicNpcs = {"#cMaps#", "#cRecycler#", "#cOverflow#", "#cBank#", "#cAchievements#"};

    public static final int[] tutNpcIds = {9010038, 9000000, 9010000, 9000085};
    public static final String[] tutNpcsScripts = {"", ""};
    public static final String[] tutNpcs = {"#cRecycler#", "#cOverflow#", "#cBank#", "#cAchievements#"};
    ...
    public static final double VIEW_RANGE = Double.POSITIVE_INFINITY;
    ...
    public static void setLock(boolean toggle) {
        lock = toggle;
    }
    ...
    public static boolean isCurrency(int id) {
        return Math.floor(id / 10000.0) == 431;
    }

    public static int getShopCurrency(int id) {
        switch (id) {
            case 5140000://standard shop
                return 4310999;
            case 5140001://MPC shop
                return 4310020;
            case 5140003://GE shop
                return 4310015;
            case 5140004://VP shop
                return 4310503;
            case 5140006://MP shop
                return 4310501;
            case 5140007://DP shop
                return 4310502;
            case 5140008://Golden Bag Shop
                return 4310500;
            case 5140009://Casino Shop
                return 4310101;
            case 5140010://Crypto Shop
                return 4310504;
            case 5140011://CC shop
                return 4310018;
            case 5140012://doll shop
                return 4310510;
            case 5140013://Crypto Shop
                return 4310505;
        }
        return 4310999;
    }

    public static int getOrbsFromTier(int tier) {
        switch (tier) {
            case 0://common
            case 1://uncommon
            case 2://rare
    ...
    public static Pair<Integer, Integer> getAttack(int skillid) {
        Pair<Integer, Integer> attacks = new Pair<>(1, 1);
        if (getAttackSkill(skillid) != null) {
            attacks = getAttackSkill(skillid);
        }
        //else {
        //    if (skill_attacks.containsKey(skillid)) {
        //       int attackz = Randomizer.random(1, 8);
        //        int linez = Randomizer.random(1, 8);
        //        GameConstants.addAttackSkill(skillid, attackz, linez);
        //    }
        //}
        return attacks;
    }

    public static boolean canWearEquip(MapleClient c, Equip eqp) {
        boolean wear = false;
        if (!GameConstants.isGMJob(c.getPlayer().getJob())) {
            final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            final Map<String, Integer> stats = ii.getEquipStats(eqp.getItemId());
            if (stats.get("reqJob") != null && stats.get("reqJob") > 0) {
                if (GameConstants.isWarriorJob(c.getPlayer().getJob())) {
                    if (stats.get("reqJob") == 1 || stats.get("reqJob") == 3 || stats.get("reqJob") == 5 || stats.get("reqJob") == 9 || stats.get("reqJob") == 9 || stats.get("reqJob") == 13 || stats.get("reqJob") == 17) {
                        wear = true;
                    }
                }
                if (GameConstants.isMageJob(c.getPlayer().getJob())) {
                    if (stats.get("reqJob") == 2 || stats.get("reqJob") == 3 || stats.get("reqJob") == 10) {
                        wear = true;
                    }
                }
                if (GameConstants.isBowmanJob(c.getPlayer().getJob())) {
                    if (stats.get("reqJob") == 4 || stats.get("reqJob") == 5 || stats.get("reqJob") == 13) {
                        wear = true;
                    }
                }
                if (GameConstants.isThiefJob(c.getPlayer().getJob())) {
                    if (stats.get("reqJob") == 8 || stats.get("reqJob") == 9 || stats.get("reqJob") == 10 || stats.get("reqJob") == 13 || stats.get("reqJob") == 24) {
                        wear = true;
                    }
                }
                if (GameConstants.isPirateJob(c.getPlayer().getJob())) {
                    if (stats.get("reqJob") == 16 || stats.get("reqJob") == 17) {
                        wear = true;
                    }
                }
                if (stats.get("reqJob") == 31) {
                    wear = true;
                }
            } else {
                wear = true;
    ...
    public static boolean isItemBuff(int itemid) {
        ...
        switch (itemid) {//kaotic buff timer
            case 2450020, 2450023, 2450024, 2450025, 2450050, 2450051, 2450052, 2450053 ->
                buff = true;
        }
        return buff;
    }
