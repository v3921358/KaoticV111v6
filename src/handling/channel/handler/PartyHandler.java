...
public class PartyHandler {
    ...
    public static final void DenyPartyRequest(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        if (World.Raid.getRaid(partyid) != null) {

            if (c.getPlayer().getRaid() == null && c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                MapleRaid rad = World.Raid.getRaid(partyid);
                if (action == (GameConstants.GMS ? 0x1F : 0x1B)) { //accept
                    rad.addMember(c.getPlayer());
                } else if (action != (GameConstants.GMS ? 0x1E : 0x16)) {
                    final MapleCharacter cfrom = c.getChannelServer().getPlayerStorage().getCharacterById(rad.getLeader().getId());
                    if (cfrom != null) { // %s has denied the party request.
                        cfrom.getClient().announce(PartyPacket.partyStatusMessage(23, c.getPlayer().getName()));
                    }
                }
            }
        } else {
            ...
            if (c.getPlayer().getParty() == null && c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                ...
                if (party != null) {
                    ...
                    c.getPlayer().dropMessage(5, "The party you are trying to join does not exist TEST 2");
                }
            } else {
    ...
    public static final void PartyOperation(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        final int operation = slea.readByte();
        MapleParty party = c.getPlayer().getParty();
        MapleRaid raid = c.getPlayer().getRaid();
            ...
            case 1: // create
                if (raid == null) {
                    if (party == null) {
                        ...
                        c.getPlayer().setParty(party);
                        c.announce(PartyPacket.partyCreated(party.getId()));
                    } else {
                        if (party.getExpeditionId() > 0) {
                            c.getPlayer().dropMessage(5, "You may not do party operations while in a raid.");
                        } else {
                    ...
                    c.getPlayer().dropMessage(5, "You can't create a party as you are already in a raid");
                }
                break;
            case 2: // leave
            ...
            case 3: // accept invitation
                ...
                if (World.Raid.getRaid(partyid) != null) {
                    ...
                    if (c.getPlayer().getRaid() == null && c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                        ...
                        c.getPlayer().dropMessage(5, "Error with raid invite.");
                    }
                } else {
                    if (party == null) {
                        ...
                        if (party != null) {
                            ...
                            if (party.getMembers().size() < GameConstants.getPartySize() && c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                                c.getPlayer().setParty(party);
                            ...
                            c.getPlayer().dropMessage(5, "The party you are trying to join does not exist TEST");
                        }
                    } else {
                        c.getPlayer().dropMessage(5, "You can't join the party as you are already in one");
                    }
                ...
                break;
            case 4: // invite
                boolean getRaid = false;
                if (party == null && c.getPlayer().getRaid() == null) {
                    party = World.Party.createParty(partyplayer);
                    c.getPlayer().setParty(party);
                    c.announce(PartyPacket.partyCreated(party.getId()));
                }
                ...
                if (theCh > 0) {
                    ...
                    if (invited != null) {
                        if (getRaid) {
                            if (invited.getParty() == null && invited.getRaid() == null) {
                                if (c.getPlayer().getRaid().getLeader() == c.getPlayer()) {
                                    c.getPlayer().dropMessage(5, invited.getName() + " has been invited to join the raid.");
                                    //invited.getClient().announce(PartyPacket.raidInvite(c.getPlayer()));
                                    invited.getClient().announce(ExpeditionPacket.expeditionInvite(c.getPlayer(), 2009));
                                } else {
                                    c.getPlayer().dropMessage(5, "Only Raid Leader can invite other players.");
                                }
                            } else {
                                c.getPlayer().dropMessage(5, invited.getName() + " is already in a party or raid.");
                            }
                        } else {
                            if (party != null && invited.getParty() == null && invited.getRaid() == null && invited.getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                                if (party.getExpeditionId() > 0) {
                                    c.getPlayer().dropMessage(5, "You may not do party operations while in a raid.");
                                    return;
                                }
                                if (party.getMembers().size() < GameConstants.getPartySize()) {
                                    c.announce(PartyPacket.partyStatusMessage(26, invited.getName()));
                                    invited.getClient().announce(PartyPacket.partyInvite(c.getPlayer()));
                                } else {
                                    c.announce(PartyPacket.partyStatusMessage(22, null));
                                }
                            } else {
                                c.announce(PartyPacket.partyStatusMessage(21, null));
                            }
                        }
                    } else {
                        c.getPlayer().dropMessage(1, "Player you tried to invite is offline or does not exist.");
                    }
                } else {
                    c.getPlayer().dropMessage(1, "Player you tried to invite is offline or does not exist.");
                }
                ...
                if (GameConstants.GMS) {
                    ...
                    if (party != null && party.getMembers().size() < GameConstants.getPartySize()) {
                        ...
                        if (cfrom != null && cfrom.getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_REQUEST)) == null) {
                            c.announce(PartyPacket.partyStatusMessage(50, c.getPlayer().getName()));
    ...
    public static final void AllowPartyInvite(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() != null) {
            if (slea.readByte() > 0) {
    ...
    public static final void MemberSearch(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() != null) {
            ...
            c.announce(PartyPacket.showMemberSearch(c.getPlayer().getMap().getCharacters()));
        }
    }

    public static final void PartySearch(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        c.getPlayer().dropMessage(5, "Party search is currently disabled. pls find friends.");
        /*
         if (c.getPlayer().isInBlockedMap() || FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())) {
         c.getPlayer().dropMessage(5, "You may not do party search here.");
         return;
    ...
    public static final void PartyListing(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (mode) {
            ...
            case -105:
                ...
                if (c.getPlayer().getParty() == null && World.Party.searchParty(pst).size() < 10) {
                    ...
                    if (pst.exped) {
                        c.announce(ExpeditionPacket.expeditionStatus(World.Party.getExped(party.getExpeditionId()), true));
                    }
            ...
            case -101:
                ...
                if (party == null) { //are we in a party? o.O"
                    ...
                    if (party != null) {
                        ...
                        if (ps != null && c.getPlayer().getLevel() <= ps.getType().maxLevel && c.getPlayer().getLevel() >= ps.getType().minLevel && party.getMembers().size() < GameConstants.getPartySize()) {
                            c.getPlayer().setParty(party);
                            World.Party.updateParty(party.getId(), PartyOperation.JOIN, partyplayer);
                            c.getPlayer().receivePartyMemberHP();
                            c.getPlayer().updatePartyMemberHP();
                        ...
                        if (exped != null) {
                            ...
                            if (ps != null && c.getPlayer().getLevel() <= ps.getType().maxLevel && c.getPlayer().getLevel() >= ps.getType().minLevel && exped.getAllMembers() < exped.getType().maxMembers) {
                                ...
                                if (partyId < 0) {
                                    ...
                                    c.getPlayer().setParty(party);
                                    c.announce(PartyPacket.partyCreated(party.getId()));
                                    c.announce(ExpeditionPacket.expeditionStatus(exped, true));
                                ...
                                c.announce(ExpeditionPacket.expeditionError(0, c.getPlayer().getName()));
                            }
                        }
                    }
                }
    ...
    public static final void Expedition(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (mode) {
            ...
            case 134:
                ...
                if (et != null && c.getPlayer().getParty() == null && c.getPlayer().getLevel() <= et.maxLevel && c.getPlayer().getLevel() >= et.minLevel) {
                    ...
                    c.announce(ExpeditionPacket.expeditionError(0, ""));
                }
                break;
            case 65: //invite [name]
            case 135:
                ...
                if (theCh > 0) {
                    ...
                    if (invited != null && invited.getParty() == null && party != null && party.getExpeditionId() > 0) {
                        ...
                        if (me != null && me.getAllMembers() < me.getType().maxMembers && invited.getLevel() <= me.getType().maxLevel && invited.getLevel() >= me.getType().minLevel) {
                            c.announce(ExpeditionPacket.expeditionError(7, invited.getName()));
                            invited.getClient().announce(ExpeditionPacket.expeditionInvite(c.getPlayer(), me.getType().exped));
                        } else {
                            c.announce(ExpeditionPacket.expeditionError(3, invited.getName()));
                        }
                    } else {
                        c.announce(ExpeditionPacket.expeditionError(2, name));
                    }
                } else {
                    c.announce(ExpeditionPacket.expeditionError(0, name));
                }
                break;
            ...
            case 136:
                ...
                if (slea.readByte() == 8) {
                    ...
                    if (cfrom.getRaid() != null && cfrom.getRaid().getLeader(cfrom)) {
                        cfrom.getRaid().addMember(player);
                        player.getQuestRemove(MapleQuest.getInstance(GameConstants.PARTY_INVITE));
                    }
                }
                ...
                /*
                 ...
                 c.announce(ExpeditionPacket.expeditionError(3, cfrom.getName()));
                 }
                 } else if (action == 9) {
                 cfrom.getClient().announce(PartyPacket.partyStatusMessage(23, c.getPlayer().getName()));
                 }
                                    ...
                                    if (expelled != null && expelled.isOnline()) {
                                        ...
                                        if (partyIndexTo < exped.getParties().size()) { //already exists
                                            ...
                                            if (party != null && party.getMembers().size() < GameConstants.getPartySize()) {
                                                ...
                                                chr.receivePartyMemberHP();
                                                chr.updatePartyMemberHP();
                                                chr.getClient().announce(ExpeditionPacket.expeditionStatus(exped, true));
                                            }
                                        } else {
                                            party = World.Party.createPartyAndAdd(expelled, exped.getId());
                                            chr.setParty(party);
                                            chr.getClient().announce(PartyPacket.partyCreated(party.getId()));
                                            chr.getClient().announce(ExpeditionPacket.expeditionStatus(exped, true));
                                            World.Party.expedPacket(exped.getId(), ExpeditionPacket.expeditionUpdate(exped.getIndex(party.getId()), party), null);
                                        }
                                        if (c.getPlayer().getEventInstance() != null) {
