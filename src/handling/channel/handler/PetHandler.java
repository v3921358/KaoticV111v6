...
public class PetHandler {
    ...
    public static final void PetFood(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getPets().isEmpty()) {
            chr.dropMessage("You have no pets!");
            c.announce(CWvsContext.enableActions());
            return;
        }
        if (chr.haveItem(2120000)) {
            for (final MaplePet pets : chr.getPets()) {
                if (pets != null) {
                    if (pets.getSummoned()) {
                        if (pets.getFullness() < 100) {
                            pets.setFullness(100);
                            c.announce(CWvsContext.InventoryPacket.updateItemslot(chr.getInventory(MapleInventoryType.CASH).getItem((byte) pets.getInventoryPosition())));
                            chr.getMap().broadcastMessage(c.getPlayer(), PetPacket.petFoodResponse(chr.getId(), chr.getPetIndex(pets), true, false), true);
                            //chr.getClient().getSession().write(PetPacket.updatePet(pets, chr.getInventory(MapleInventoryType.CASH).getItem(pets.getInventoryPosition()), true));
                            pets.saveToDb();
                        }
                    }
                }
            ...
            if (itemId == 2120000) {
                chr.dropBuff += 3600;
                chr.dropMessage("1 Hour of Pet attack mode has been added to account!");
            }
        }
        c.announce(CWvsContext.enableActions());
    }
    ...
    public static final void MovePet(final SeekableLittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        final List<LifeMovementFragment> res = MovementParse.parseMovement(chr, slea, 3);
        if (res != null && chr != null && res.size() != 0 && chr.getMap() != null) { // map crash hack
            ...
            if (chr.getMap().allowPets && !pet.isStatLock()) {
                if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit())) {
                    if (chr.isAlive()) {
                        chr.dispelDebuffs();
                        if (chr.getStat().getHp() < chr.getStat().getCurrentMaxHp()) {
                            chr.healHP((int) (chr.getStat().getCurrentMaxHp() * 0.1));
                        }
                        if (chr.getStat().getMp() < chr.getStat().getCurrentMaxMp(chr.getJob())) {
                            chr.healMP((int) (chr.getStat().getCurrentMaxMp(chr.getJob()) * 0.1));
                        }
                    }
                    pet.setStatLock(System.currentTimeMillis() + 100);
                }
            }
            ...
            List<MapleMapObject> objects = chr.getMap().getMapObjectsInRange(chr.getTruePosition(), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.ITEM));
