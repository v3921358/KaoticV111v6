...
public class MapleGuild implements java.io.Serializable {
    ...
    private int id, logo, logoColor, leader, capacity, logoBG, logoBGColor, signature, level;
    private long gp;
    ...
    public MapleGuild(final int guildid, Map<Integer, Map<Integer, MapleBBSReply>> replies) {
        ...
        try ( Connection con = DatabaseConnection.getPlayerConnection()) {
            ...
            ps = con.prepareStatement("SELECT id, name, totallevel, job, guildrank, guildContribution, alliancerank FROM characters WHERE guildid = ? ORDER BY guildrank ASC, name ASC", ResultSet.CONCUR_UPDATABLE);
            ps.setInt(1, guildid);
            rs = ps.executeQuery();
            ...
            do {
                ...
                members.add(new MapleGuildCharacter(cid, rs.getInt("totallevel"), rs.getString("name"), (byte) -1, rs.getInt("job"), gRank, rs.getInt("guildContribution"), aRank, guildid, false));
            } while (rs.next());
            rs.close();
            ps.close();
    ...
    public long getBaseGP() {
        long gpoint = 0;
        for (MapleGuildCharacter chr : getMembers()) {
            gpoint += chr.getGuildContribution();
        }
        return gpoint;
    }
    ...
    public final boolean increaseCapacity(boolean trueMax) {
        if (capacity >= 250 || ((capacity + 5) > 250)) {
            return false;
        }
        if (trueMax && gp < 25000) {
            return false;
    ...
    public final void gainGP(int amount, final boolean broadcast, final int cid) {
        ...
        if (cid > 0 && amount > 0) {
            ...
            if (mg != null && (mg.getGuildContribution() < Integer.MAX_VALUE)) {
                if (mg.getGuildContribution() < 0) {
                    mg.setGuildContribution(Integer.MAX_VALUE);
                } else {
                    int total = Randomizer.Max(mg.getGuildContribution() + amount, Integer.MAX_VALUE);
                    mg.setGuildContribution(total);
                }
                if (mg.isOnline()) {
        ...
        gp += amount;
        level = calculateLevel();
        broadcast(GuildPacket.updateGP(id, (int) Randomizer.MaxLong(gp, Integer.MAX_VALUE), level));
        if (broadcast) {
            broadcast(CWvsContext.yellowChat("Your guild has received " + amount + " Guild Points"));
        }
    }
    ...
    public boolean purchaseSkill(int skill, String name, int cid) {
        ...
        try ( Connection con = DatabaseConnection.getPlayerConnection();  PreparedStatement ps = con.prepareStatement(""
    ...
    public final int calculateLevel() {
        for (int i = 1; i < GameConstants.getGuildMaxLevel(); i++) {
            if (gp < GameConstants.getGuildExpNeededForLevel(i)) {
                return i;
            }
        }
        return GameConstants.getGuildMaxLevel();
    }

    public final int calculateExp(int level) {
        return GameConstants.getGuildExpNeededForLevel(level);
    }
    ...
    public final void addMemberData(final MaplePacketLittleEndianWriter mplew) {
        ...
        for (final MapleGuildCharacter mgc : members) {
            ...
            mplew.writeInt(Randomizer.Max(mgc.getGuildContribution(), Integer.MAX_VALUE));
        }
    }
