...
public class GenericLittleEndianWriter implements LittleEndianWriter {
    ...
    /**
     * Writes a float to the stream. (Special cases may be handled differently by client)
     */
    ...
    public void writeString(String s, int length) {
    	...
    	bos.write(bytes, 0, Math.min(bytes.length, length));
    	writeZeroBytes(Math.max(0, length - s.length()));
    }
