...
public class DumpItems {
    ...
    final MapleDataProvider string = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz"));
    private MapleDataProvider item, character;
    ...
    protected int id = 0;
    private Connection con = DatabaseConnection.getWorldConnection();
    ...
    public void dumpItems() throws Exception {
        if (!hadError) {
            PreparedStatement psa = con.prepareStatement("INSERT INTO wz_itemadddata_dump(itemid, `key`, value1, value2) VALUES (?, ?, ?, ?)");
            PreparedStatement psr = con.prepareStatement("INSERT INTO wz_itemrewarddata_dump(itemid, item, prob, quantity, period, worldMsg, effect) VALUES (?, ?, ?, ?, ?, ?, ?)");
            PreparedStatement ps = con.prepareStatement("INSERT INTO wz_itemdata_dump(itemid, name, msg, `desc`, slotMax, price, wholePrice, stateChange, flags, karma, meso, monsterBook, itemMakeLevel, questId, scrollReqs, consumeItem, totalprob, incSkill, replaceId, replaceMsg, `create`, afterImage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            PreparedStatement pse = con.prepareStatement("INSERT INTO wz_itemequipdata_dump(itemid, itemLevel, `key`, `value`) VALUES (?, ?, ?, ?)");
            try {
    ...
    public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, MapleData iz) throws Exception {
        ...
        doneIds.add(id);
        if (update && doesExist("SELECT * FROM wz_itemdata_dump WHERE itemid = " + id)) {
            return;
        }
        ps.setInt(1, id);
    ...
    public void dumpItems(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse) throws Exception {
        if (!update) {
            delete("DELETE FROM wz_itemdata_dump");
            delete("DELETE FROM wz_itemequipdata_dump");
            delete("DELETE FROM wz_itemadddata_dump");
            delete("DELETE FROM wz_itemrewarddata_dump");
