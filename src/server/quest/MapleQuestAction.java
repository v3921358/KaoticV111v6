    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            ...
            case nextQuest:
                ...
                c.getClient().announce(CField.updateQuestFinish(quest.getId(), status.getNpc(), intStore));
                break;
            case money:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
            ...
            case skill:
                ...
                for (Triple<Integer, Integer, Integer> skills : skill) {
                    ...
                    if (skillObject.isBeginnerSkill() || found) {
                        sa.put(skillObject, new SkillEntry(Math.max(skillLevel, c.getSkillLevel(skillObject)), Math.max(masterLevel, c.getMasterLevel(skillObject)), SkillFactory.getDefaultSExpiry(skillObject)));
                    }
                }
            ...
            case pop:
                ...
                c.getClient().announce(InfoPacket.getShowFameGain(fameGain));
                break;
            case buffItemID:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        switch (type) {
            ...
            case nextQuest: {
                c.getClient().announce(CField.updateQuestFinish(quest.getId(), c.getQuest(quest).getNpc(), intStore));
                break;
            }
            case money: {
                c.gainMeso(intStore, true, true);
