...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
    ...
    public PlayerNPC(ResultSet rs, Connection con) throws Exception {
    ...
    public static void loadAll() {
        ...
        try (Connection con = DatabaseConnection.getPlayerConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM playernpcs"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                toAdd.add(new PlayerNPC(rs, con));
            }
        } catch (Exception se) {
            se.printStackTrace();
        }
    ...
    public void destroy(boolean remove) {
        try (Connection con = DatabaseConnection.getPlayerConnection(); PreparedStatement ps = con.prepareStatement("DELETE FROM playernpcs WHERE scriptid = ?")) {
            ps.setInt(1, getId());
            ps.executeUpdate();
        } catch (Exception se) {
            se.printStackTrace();
    ...
    public void saveToDB() {
        try (Connection con = DatabaseConnection.getPlayerConnection()) {
            ...
            destroy();
            try (PreparedStatement ps = con.prepareStatement("INSERT INTO playernpcs(name, hair, face, skin, x, y, map, charid, scriptid, foothold, dir, gender, pets) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
