    ...
    public void run() {
        for (ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (MapleCharacter player : cserv.getPlayerStorage().getAllCharacters()) {
                if (player != null && !player.getLock() && player.isAlive() && player.getEventInstance() == null && player.isCombat() && player.getClient().getCMS() == null) {
                    NPCScriptManager.getInstance().startNPC(player.getClient(), 9010106, "bot");
                }
            }
        }
