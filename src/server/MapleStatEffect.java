...
public class MapleStatEffect implements Serializable {
    ...
    private byte mastery, mobCount, attackCount, bulletCount, reqGuildLevel, period, familiarTarget, iceGageCon, //1 = party 2 = nearby
            recipeUseCount, recipeValidDay, reqSkillLevel, slotCount, effectedOnAlly, effectedOnEnemy, type, preventslip, immortal, bs;
    private short hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, psdSpeed, psdJump, forceCon, bdR, damage, prop,
            ...
            mobSkill, mobSkillLevel, mhpR, mmpR, actionSpeed; //ar = accuracy rate
    private double hpR, mpR, statR, wdefX, mdefX, strR, dexR, intR, lukR, watkR, matkR, wdefR, mdefR;
    ...
    private int duration, subTime, sourceid, recipe, moveTo, t, u, v, w, x, y, z, cr, itemCon, itemConNo, bulletConsume, moneyCon,
            cooldown, morphId = 0, expinc, exp, consumeOnPickup, range, price, extendPrice, charColor, interval, rewardMeso, totalprob, cosmetic, overpower, stats, resist, atkR, etcR, BuffR, mpCon, hpCon;
    ...
    private Point lt, rb;
    private int kpBuff, expBuff, itemup, mesoup, cashup, berserk, illusion, booster, berserk2, cp, nuffSkill, expR, dropR, masterBuff;
    private int level;
//    private List<Pair<Integer, Integer>> randomMorph;
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final int level, final String variables) {
        ...
        ret.actionSpeed = (short) parseEval("actionSpeed", source, 0, variables, level);
        ...
        ret.statR = parseEval("allStat", source, 0, variables, level);
        ...
        ret.mpCon = parseEval("mpCon", source, 0, variables, level);
        ret.hpCon = parseEval("hpCon", source, 0, variables, level);
        ...
        ret.mobCount = (byte) Randomizer.Max(parseEval("mobCount", source, 1, variables, level), 15);
        ...
        ret.expR = parseEval("expR", source, 0, variables, level);
        ret.dropR = parseEval("dropR", source, 0, variables, level);
        ...
        ret.attackCount = (byte) Randomizer.Max(parseEval("attackCount", source, 1, variables, level), 15);
        ret.bulletCount = (byte) Randomizer.Max(parseEval("bulletCount", source, 1, variables, level), 15);
        ...
        if (!ret.skill && ret.duration > -1) {
            ...
            ret.duration = ret.duration * 1000; // items have their times stored in ms, of course
            ret.subTime = ret.subTime * 1000;
        ...
        ret.wdefX = (short) parseEval("pddX", source, 0, variables, level);
        ret.mdefX = (short) parseEval("mddX", source, 0, variables, level);
        ...
        ret.watkR = (short) parseEval("padR", source, 0, variables, level);
        ret.matkR = (short) parseEval("madR", source, 0, variables, level);
        ret.wdefR = (short) parseEval("pddR", source, 0, variables, level);
        ret.mdefR = (short) parseEval("mddR", source, 0, variables, level);
        ret.expBuff = parseEval("expBuff", source, 0, variables, level);
        ret.kpBuff = parseEval("kpBuff", source, 0, variables, level);
        ret.masterBuff = parseEval("masterBuff", source, 0, variables, level);
        ...
        ret.overpower = parseEval("overpower", source, 0, variables, level);
        ret.stats = parseEval("statR", source, 0, variables, level);
        ret.resist = parseEval("resistR", source, 0, variables, level);
        ret.BuffR = parseEval("buffR", source, 0, variables, level);
        ret.atkR = parseEval("atkR", source, 0, variables, level);
        ret.etcR = parseEval("etcR", source, 0, variables, level);
        ...
        if (ret.overTime && ret.getSummonMovementType() == null && !ret.isEnergyCharge()) {
            ...
            addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.MASTERRATE, Integer.valueOf(ret.masterBuff)); // EXP
            addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.KPRATE, Integer.valueOf(ret.kpBuff)); // KP
            ...
            if (sourceid == 5221006 || ret.isPirateMorph() || sourceid == 33001001) { //HACK: add stance :D and also this buffstat has to be the first one..
        ...
        if (ret.skill) { // hack because we can't get from the datafile...
            ...
            switch (sourceid) {
                ...
                case 30010190:
                    ret.statups.put(MapleBuffStat.STANCE, 100); //100% :D:D:D
                    ret.statups.put(MapleBuffStat.BOOSTER, (int) ret.actionSpeed);
                    break;
                case 1101006:
                case 2101001:
                case 2201001:
                case 12101000:
                case 11101003:
                    ret.statups.put(MapleBuffStat.DAMAGE_BUFF, (int) ret.damR); //i think
                    break;
                ...
                case 21000000: // Aran Combo
                    ret.statups.put(MapleBuffStat.ARAN_COMBO, 10);
                    break;
                case 23101003:
    ...
    public final boolean applyTo(MapleCharacter chr) {
        return applyTo(chr, chr, true, null, getDuration());
    }

    public final boolean applyTo(MapleCharacter chr, Point pos) {
        return applyTo(chr, chr, true, pos, getDuration());
    }

    public final boolean applyTo(MapleCharacter chr, Point pos, boolean cost) {
        return applyTo(chr, chr, true, pos, getDuration(), cost);
    }
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        return applyTo(applyfrom, applyto, primary, pos, getDuration(), true);
    }

    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration, boolean cost) {
        ...
        int hpchange = calcHPChange(applyfrom, primary, cost);
        int mpchange = calcMPChange(applyfrom, primary, cost);
        ...
        if (primary) {
            ...
            if (isResurrection()) {
                hpchange = applyto.getStat().getCurrentMaxHp();
                mpchange = applyto.getStat().getCurrentMaxMp(applyto.getJob());
                applyto.setStance(4); //TODO fix death bug, player doesnt spawn on other screen
            }
        }
        if (isDispel()) {
        ...
        if (expinc != 0) {
            ...
            applyto.getClient().announce(EffectPacket.showForeignEffect(20));
            ...
            for (MapleMist m : applyto.getMap().getAllMists()) {
                if (m.getOwnerId() == applyto.getId() && m.getSourceSkill().getId() == 2111003) {
                    ...
                    applyto.getMap().removeMapObject(m, MapleMapObjectType.MIST);
            ...
            applyto.getClient().announce(CField.getPVPIceGage(x - iceGageCon));
            ...
            if (effectedOnEnemy > 0) {
                ...
                if (sourceid == 2910000 || sourceid == 2910001) { //red flag
                    applyto.getClient().announce(EffectPacket.showOwnBuffEffect(sourceid, 13, applyto.getLevel(), level));
        ...
        if (summonMovementType != null && (sourceid != 32111006) && !applyto.isClone()) {
            if (sourceid == 3111002) {
            ...
            final MapleSummon tosummon = new MapleSummon(applyfrom, sourceid, getLevel(), new Point(pos == null ? applyfrom.getTruePosition() : pos), summonMovementType);
            ...
            if (isBeholder()) {
                ...
                for (MapleSummon s : applyfrom.getSummonsValues()) {
                ...
                applyfrom.getClient().announce(CField.skillCooldown(sourceid, getCooldown(applyfrom)));
                ...
                applyfrom.getClient().announce(CWvsContext.enableActions()); //doubt we need this at all
            }
        } else if (isMechDoor()) {
            System.out.println(applyto.getName() + " is an outdated client...");
            applyto.kick();
            applyto.getClient().announce(CWvsContext.enableActions());
            return false;
            /*
             ...
             applyto.getClient().announce(CWvsContext.mechPortal(door.getTruePosition()));
             if (!applyBuff) {
        ...
        if (isMagicDoor()) { // Magic Door - kaotic
            applyto.dropMessage(5, "Door is currently disabled.");
            applyto.getClient().announce(CWvsContext.enableActions());
            /*
             boolean active = false;
             for (MapleCharacter chr : applyfrom.getGroupMembers()) {
             if (!chr.getDoors().isEmpty()) {
             active = true;
             break;
             }
             }
             if (!active) {
             MapleDoor door = new MapleDoor(applyto, applyto.getPosition(), sourceid); // Current Map door
             if (door.getTownPortal() != null && !door.getFreePortals().isEmpty()) {
             ...
             applyto.dropMessage(5, "There is a door already active in the group");
            ...
            Point spot = applyfrom.getMap().calcPointBelow(applyfrom.getPosition());
            if (spot == null) {
                spot = applyfrom.getPosition();
            }
            final Rectangle bounds = calculateBoundingBox(pos != null ? pos : spot, applyfrom.isFacingLeft());
            ...
            for (MapleCoolDownValueHolder i : applyto.getCooldowns()) {
                if (i.skillId != 5121010) {
                    ...
                    applyto.getClient().announce(CField.skillCooldown(i.skillId, 0));
                }
            }
        }
        /*
        ...
        if (isSoulStone()) {
            if (applyfrom.getParty() != null) {
                ...
                for (MapleCharacter chr : awarded) {
                    ...
                    chr.getClient().announce(EffectPacket.showOwnBuffEffect(sourceid, 2, applyfrom.getLevel(), level));
                    chr.getMap().broadcastMessage(chr, EffectPacket.showBuffeffect(chr.getId(), sourceid, 2, applyfrom.getLevel(), level), false);
                }
            }
        } else if (isPartyBuff() && (applyfrom.getParty() != null || applyfrom.getRaid() != null || isGmBuff() || applyfrom.inPVP())) {
            ...
            for (final MapleMapObject affectedmo : affecteds) {
                ...
                if (affected.getId() != applyfrom.getId() && (isGmBuff() || (applyfrom.inPVP() && affected.getTeam() == applyfrom.getTeam() && Integer.parseInt(applyfrom.getEventInstance().getProperty("type")) != 0) || (applyfrom.getParty() != null && affected.getParty() != null && applyfrom.getParty().getId() == affected.getParty().getId()) || (applyfrom.getRaid() != null && affected.getRaid() != null && applyfrom.getRaid().getId() == affected.getRaid().getId()))) {
                    if ((isResurrection() && !affected.isAlive()) || (!isResurrection() && affected.isAlive())) {
                        applyTo(applyfrom, affected, false, null, newDuration, false);
                        affected.getClient().announce(EffectPacket.showOwnBuffEffect(sourceid, 2, applyfrom.getLevel(), level));
                    ...
                    if (isTimeLeap()) {
                        for (MapleCoolDownValueHolder i : affected.getCooldowns()) {
                            if (i.skillId != 5121010) {
                                ...
                                affected.getClient().announce(CField.skillCooldown(i.skillId, 0));
                            }
                        }
                    }
                }
    ...
    private void removeMonsterBuff(final MapleCharacter applyfrom) {
        for (final MapleMonster mob : applyfrom.getMap().getAllMonsters()) {
            if (makeChanceResult()) {
                if (mob != null && mob.getStatiSize() > 0) {
                    for (MonsterStatusEffect mse : mob.getAllBuffs()) {
                        if (mse.getCancelTask() > 0) {
                            mob.cancelSingleStatus(mse);
                        }
                    }
                }
            }
    ...
    public final void handleExtraPVP(MapleCharacter applyfrom, MapleCharacter chr) {
        if (sourceid == 2311005 || sourceid == 5121005 || sourceid == 1201006 || (GameConstants.isBeginnerJob(sourceid / 10000) && sourceid % 10000 == 104)) { //doom, threaten, snatch
            ...
            chr.getClient().announce(BuffPacket.giveBuff(localsourceid, getDuration(), localstatups, this));
    ...
    public final void applyComboBuff(final MapleCharacter applyto, int combo) {
        ...
        stat.put(MapleBuffStat.ARAN_COMBO, combo);
        applyto.getClient().announce(BuffPacket.giveBuff(sourceid, 99999, stat, this)); // Hackish timing, todo find out
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity) {
        final long starttime = System.currentTimeMillis();
        if (infinity) {
            applyto.getClient().announce(BuffPacket.giveEnergyChargeTest(0, getDuration() / 1000));
            applyto.registerEffect(this, starttime, null, applyto.getId());
        } else {
            final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
            stat.put(MapleBuffStat.ENERGY_CHARGE, 10000);
            applyto.cancelEffect(this, true, -1, stat);
            applyto.getMap().broadcastMessage(applyto, BuffPacket.giveEnergyChargeTest(applyto.getId(), 10000, getDuration() / 1000), false);
            final CancelEffectAction cancelAction = new CancelEffectAction(applyto, this, starttime, stat);
            final ScheduledFuture<?> schedule = BuffTimer.getInstance().schedule(cancelAction, ((starttime + getDuration()) - System.currentTimeMillis()));
            applyto.registerEffect(this, starttime, schedule, stat, false, getDuration(), applyto.getId());

        }
    }

    public final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        int localDuration = newDuration;
        boolean itembuff = GameConstants.isItemBuff(sourceid);
        if (primary) {
        ...
        switch (sourceid) {
            ...
            case 15111011: {//dice
                ...
                applyto.getClient().announce(EffectPacket.showOwnDiceEffect(sourceid, zz, -1, level));
                if (zz <= 1) {
                    return;
                }
            ...
            case 5320007: {//dice
                ...
                applyto.getClient().announce(EffectPacket.showOwnDiceEffect(sourceid, zz, zz2 > 0 ? -1 : 0, level));
                if (zz <= 1 && zz2 <= 1) {
                    return;
                }
            ...
            case 2320011: //arcane aim
                if (applyto.getFirstLinkMid() > 0) {
                    applyto.getClient().announce(BuffPacket.giveArcane(applyto.getAllLinkMid(), localDuration));
                } else {
                    return;
                }
                normal = false;
            ...
            case 32121003: { //twister
                ...
                applyto.getMap().broadcastSkill(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                break;
            }
            case 32111005: { //body boost
                ...
                if (applyfrom.getStatForBuff(MapleBuffStat.DARK_AURA) != null) {
                    sourcez = 32001003;
                    statt = new Pair<MapleBuffStat, Integer>(MapleBuffStat.DARK_AURA, (int) (applyto.getTotalSkillLevel(32001003))); //i think
                ...
                applyto.getClient().announce(BuffPacket.giveBuff(sourceid, localDuration, localstatups, this));
                localstatups.put(statt.left, statt.right);
                final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                stat.put(statt.left, statt.right);
                applyto.getClient().announce(BuffPacket.giveBuff(sourcez, localDuration, stat, this));
                normal = false;
                break;
            }
            case 32001003: {//dark aura
            ...
            case 32120000: { // adv dark aura
                ...
                applyto.getClient().announce(BuffPacket.giveBuff(sourceid == 32120000 ? 32001003 : sourceid, localDuration, statt, this));
                statt.clear();
                statt.put(MapleBuffStat.DARK_AURA, level);
                applyto.getClient().announce(BuffPacket.giveBuff(sourceid, localDuration, statt, this));
            ...
            case 32110000: { // advanced blue aura
                ...
                applyto.getClient().announce(BuffPacket.giveBuff(sourceid == 32110000 ? 32111012 : sourceid, localDuration, statt, this));
                statt.clear();
            ...
            case 32120001: { // advanced yellow aura
                ...
                applyto.getClient().announce(BuffPacket.giveBuff(sourceid == 32120001 ? 32101003 : sourceid, localDuration, statt, this));
                statt.clear();
            ...
            default:
                if (isPirateMorph()) {
                    ...
                    //stat.put(MapleBuffStat.JUMP, (int) jump);
                    //stat.put(MapleBuffStat.SPEED, (int) speed);
                    //stat.put(MapleBuffStat.ENHANCED_WDEF, (int) ewdef);
                    //stat.put(MapleBuffStat.ENHANCED_MDEF, (int) emdef);
                    applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                    applyto.getClient().announce(BuffPacket.giveBuff(sourceid, localDuration, stat, this));
                    maskedStatups = new EnumMap<MapleBuffStat, Integer>(localstatups);
                    maskedStatups.remove(MapleBuffStat.MORPH);
                    //maskedStatups.remove(MapleBuffStat.JUMP);
                    //maskedStatups.remove(MapleBuffStat.SPEED);
                    //maskedStatups.remove(MapleBuffStat.ENHANCED_WDEF);
                    //maskedStatups.remove(MapleBuffStat.ENHANCED_MDEF);
                    normal = false;
                } else if (isMorph()) {
                    if (applyto.isHidden()) {
                        break;
                    ...
                    if (isIceKnight()) {
                        ...
                        applyto.getClient().announce(BuffPacket.giveBuff(0, localDuration, stat, this));
                    }
                    final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                    stat.put(MapleBuffStat.MORPH, getMorph(applyto));
                    applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
        ...
        if (showEffect && !applyto.isHidden()) {
            applyto.getMap().broadcastSkill(applyto, EffectPacket.showBuffeffect(applyto.getId(), sourceid, 1, applyto.getLevel(), level), false);
        }
        if (isMechPassive()) {
            applyto.getClient().announce(EffectPacket.showOwnBuffEffect(sourceid - 1000, 1, applyto.getLevel(), level, (byte) 1));
        }
        ...
        if (normal && localstatups.size() > 0) {
            applyto.getClient().announce(BuffPacket.giveBuff((skill ? sourceid : -sourceid), localDuration, maskedStatups == null ? localstatups : maskedStatups, this));
        }
        final long starttime = System.currentTimeMillis();
    ...
    private final int calcHPChange(final MapleCharacter applyfrom, final boolean primary, boolean cost) {
        int hpchange = 0;
        if (hp != 0) {
            if (!skill) {
                if (primary) {
    ...
    private final int calcMPChange(final MapleCharacter applyfrom, final boolean primary, boolean cost) {
        ...
        if (primary) {
            if (mpCon != 0 && !GameConstants.isDemon(applyfrom.getJob())) {
                if (applyfrom.getBuffedValue(MapleBuffStat.INFINITY) != null) {
                    ...
                    int mpcost = (int) ((mpCon - (mpCon * applyfrom.getStat().mpconReduce / 100)) * (applyfrom.getStat().mpconPercent / 100.0));
                    mpchange -= cost ? mpcost : 0;
                }
            } else if (forceCon != 0 && GameConstants.isDemon(applyfrom.getJob())) {
                if (applyfrom.getBuffedValue(MapleBuffStat.BOUNDLESS_RAGE) != null) {
                    mpchange = 0;
                } else {
                    mpchange -= cost ? forceCon : 0;
                    //mpchange -= forceCon;
                }
            }
        }
    ...
    public final int alchemistModifyVal(final MapleCharacter chr, final int val, final boolean withX) {
        long base = (long) (val * (100.0 + (withX ? chr.getStat().RecoveryUP : ((double) (chr.getStat().BuffUP_Skill) + (getSummonMovementType() == null ? 0 : chr.getStat().BuffUP_Summon)))) / 100.0);
        if (!skill) { // RecoveryUP only used for hp items and skills
            base = (long) ((val * (100.0 + (withX ? chr.getStat().RecoveryUP : (double) (chr.getStat().BuffUP))) / 100) * ((double) (chr.getStat().BuffUP_Skill) / 100.0));
        }
        return (int) Randomizer.LongMax(base, 4 * 3600 * 1000);
    }
    ...
    public final double getWdefX() {
        return wdefX;
    }

    public final double getMdefX() {
        return mdefX;
    }
    ...
    public final int getCooldown(final MapleCharacter chra) {
        int time = 0;
        //System.out.println("skill basecool: " + cooldown);
        //System.out.println("skill cool: " + chra.getStat().getCooldown());
        if (cooldown > 0) {
            if (chra.getStat().getCooldown() > 0) {
                time = (Randomizer.Min(cooldown - chra.getStat().getCooldown(), getMinCool()));
            }
        }
        //System.out.println("skill timecool: " + time);
        return time * 1000;
    }
    ...
    public final double getDexR() {
        return dexR;
    }

    public final double getIntR() {
        return intR;
    }

    public final double getLukR() {
        return lukR;
    }

    public final double getWatkR() {
        return watkR;
    }

    public final double getMatkR() {
        return matkR;
    }

    public final double getWdefR() {
        return wdefR;
    }

    public final double getMdefR() {
        return mdefR;
    }
