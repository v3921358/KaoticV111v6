...
function enter(pi) {
    var em = pi.getEventManager("Boss_Grove");
    if (em != null) {
        if (pi.getPlayer().getStamina() >= 250) {
            if (pi.getPlayer().isGroup()) {
                if (pi.getPlayer().isLeader()) {
                    if (pi.haveItem(4001063, 1)) {
                        if (em.getEligiblePartyAch(pi.getPlayer(), level, 408)) {
                            if (!em.startPlayerInstance(pi.getPlayer())) {
                                ...
                                pi.playPortalSE();
                                pi.getPlayer().removeStamina(250);
                                pi.gainItem(4001063, -1);
                                return true;
                            }
                        } else {
                        ...
                        pi.playerMessage(1, "Event requires Clouds to enter.");
                    }
                } else {
                    pi.playerMessage(1, "Must be party leader to enter.");
                }
            } else {
                pi.playerMessage(1, "Event is Party Mode.");
            }
        } else {
            pi.playerMessage(1, "You do not have enough stamina to enter the event, the cost is 250 Stamina.");
        }
    } else {
