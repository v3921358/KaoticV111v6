...
function setup(player, lvl) {
    var eim = em.newInstance(player, "End_Farm_" + player.getName(), true);
    var map = eim.getMapInstance(entryMap);
    map.setInstanced(true);
    map.setReturnMapId(exitMap);
    eim.schedule("start", 5000);
    eim.setIntProperty("finished", 0);
    eim.setIntProperty("mob", lvl);
    return eim;
}

function start(eim) {
    eim.setIntProperty("level", 2750);
    eim.setIntProperty("spawn", 50);
    var map = eim.getMapInstance(eventMapId);
    eim.broadcastPlayerMsg(5, "You have 10 minutes to kill as many monsters as you can.");
    eim.startEventTimer(600000);
    eim.schedule("waves", 1000);
    var level = eim.getIntProperty("level");
    var mob = eim.getIntProperty("mob");
    for (var i = 0; i < eim.getIntProperty("spawn"); i++) {
        map.spawnMonsterOnGroundBelow(eim.getMonster(mob, level, 4), eim.newPoint(eim.getRandom(-800, 800), 75));
    }
}
...
function waves(eim) {
    if (!eim.isDisposed()) {
        ...
        if (eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap() < eim.getIntProperty("spawn")) {
            var spawncount = 50 - eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap();
            for (var i = 0; i < spawncount; i++) {
                map.spawnMonsterOnGroundBelow(eim.getMonster(mob, level, 4), eim.newPoint(eim.getRandom(-800, 800), 75));
            }
        }
        eim.schedule("waves", 3000);
    }
}
