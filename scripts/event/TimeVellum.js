...
function setup(player) {
    var eim = em.newInstance(player, "TimeVellum_" + player.getName(), true);
...
function start(eim) {
    ...
    var map = eim.getMapInstance(eventMapId);
    map.spawnMonsterOnGroundBelow(eim.getMonster(8930100), eim.newPoint(1000, 400));
...
function bean(eim) {
    if (eim.getIntProperty("finished") < 1) {
        ...
        if (eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap() < 8) {
            ...
            map.spawnMonsterOnGround(eim.getMonsterNoAll(8930101, 325, 6), eim.newPoint(eim.getRandom(-300, 1500), 400));
        }
