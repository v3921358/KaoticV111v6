...
function setup(player, level, scale, mode) {
    var eim = em.newInstance(player, "MP_Endless_AJob" + scale + "_" + player.getName(), true);
...
function monsterKilled(mob, eim) {
    if (mob.getId() == eim.getValue("boss")) {
        if (eim.getValue("stage") < eim.getValue("last")) {
            ...
            eim.gainPartyItem(2430130, 1);
        } else {
            eim.upgradeJob();
            eim.setValue("finished", 1);
            eim.setValue("clear", 2);
            eim.victory(exitMap);
            eim.gainPartyItem(2430130, 10 + (eim.getValue("tier") * 0.1));
        }
    }
}
