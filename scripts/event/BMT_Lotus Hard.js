...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "BMT_Lotus_Hard_" + player.getName(), true);
...
function start(eim) {
    var mob = eim.getKaoticMonster(8881102, eim.getValue("level"), eim.getValue("scale"), true, false, true, true, 999, true, true, false, false);
...
function bombs(eim) {
    if (eim.getValue("finished") == 0) {
        eim.getMapInstance(eventMapId).spawnMonsterOnGroundBelow(eim.getMonsterNoAll(eim.getRandom(8240200, 8240202)), eim.newPoint(eim.getRandom(-450, 950), 750));
        eim.schedule("bombs", 100);
    }
}
