...
var exitMap = 46000;
var baseMap = 46001;
var maps = new Array(46111, 46112, 46113, 46114, 46115);
...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "DP_Endless_Elite_" + scale + "_" + player.getName(), true);
    ...
    if (mapid != eim.getValue("exit")) {
        if (eim.getValue("generate") == 0) {
            ...
            if (map != null) {
                ...
                map.parkFelSpawn(lvl, scale, 250);
            }
        }
    }
...
function monsterKilled(mob, eim) {
    var scale = eim.getValue("scale");
    if (mob.getId() == 50018) {
        eim.gainPowerExp(100, Math.pow(scale, 4));
