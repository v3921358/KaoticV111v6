...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "BMT_Magnus_" + player.getName(), true);
...
function demon(eim) {
    if (eim.getValue("finished") == 0) {
        ...
        for (var i = 1; i <= random; i++) {
            var mobid = 9001039 + eim.getRandom(0, 1);
            map.spawnMonsterOnGroundBelow(eim.getMonsterNoAll(mobid, level, scale - 2), eim.newPoint(eim.getRandom(-450, 950), 750));
        }
        eim.dropMessage(6, "[Magnus] Come forth my greatest Demons!!!");
