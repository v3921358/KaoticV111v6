...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "DP_Endless_Boss_" + scale + "_" + player.getName(), true);
...
function afterChangedMap(eim, player, mapid) {
    if (mapid != eim.getValue("exit")) {
        if (eim.getValue("generate") == 0) {
            ...
            eim.gainPartyPowerLvl(105, 1, 250, scale);
            var lvl = eim.eimLevel();
            var map = eim.getMapInstance(mapid);
            map.killAllMonsters(true);
            map.setEndless(true);
            map.setExpRate(scale);
            map.setSpawnCap(999);
            map.parkFelSpawn(lvl, scale);
        }
    }
...
function monsterKilled(mob, eim) {
    if (mob.getId() == 50007) {
        eim.gainPartyItem(4033082, 1, mob, 100, eim.getValue("scale"));
    }
    if (mob.getId() == 50008) {
        eim.gainPartyItem(4033082, 1, mob);
    }
}
