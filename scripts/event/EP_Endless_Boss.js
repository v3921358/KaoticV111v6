...
function afterChangedMap(eim, player, mapid) {
    if (mapid != eim.getValue("exit")) {
        if (eim.getValue("generate") == 0) {
            ...
            eim.gainPartyPowerLvl(100, 1, 100, scale);
            var lvl = eim.eimLevel();
            var map = eim.getMapInstance(mapid);
            if (map != null) {
                map.setEndless(true);
                map.setExpRate(scale * 25);
                map.setSpawnCap(999);
                map.setSkillExp(10.0);
                map.setPot(true);
...
function monsterKilled(mob, eim) {
    if (mob.getId() == 50017) {
        eim.gainPowerLvl(100, 1, mob, 10, 1);
    }
    if (mob.getId() == 50018) {
        eim.gainPowerLvl(100, 1, mob);
    }
}
