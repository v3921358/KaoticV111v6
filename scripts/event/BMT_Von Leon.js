...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "BMT_Von Leon_" + player.getName(), true);
...
function start(eim) {
    ...
    eim.schedule("anni", eim.getRandom(10, 30) * 1000);
    eim.schedule("anni", eim.getRandom(10, 30) * 1000);
}
...
function anni(eim) {
    if (eim.getValue("finished") == 0) {
        ...
        map.spawnMonsterOnGroundBelow(eim.getMonsterNoAll(8210011, eim.getValue("level"), eim.getValue("scale") - 1), eim.newPoint(eim.getRandom(-450, 950), 750));
...
function monsterKilled(mob, eim) {
    ...
    if (eim.getValue("finished") == 0) {
        if (mob.getId() == 8210011) {
            eim.schedule("anni", eim.getRandom(10000, 20000));
        }
    }
}
