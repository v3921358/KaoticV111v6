...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "MP_Soul_" + scale + "_" + player.getName(), true);
    eim.setIntProperty("exit", player.getMapId());
    var room = eim.getRandom(0, 16);
    if (room >= 0 && room < 5) {
        eim.setIntProperty("scale", room);
        eim.setIntProperty("base", 952000000);
    } else if (room >= 5 && room < 11) {
        eim.setIntProperty("scale", room - 5);
        eim.setIntProperty("base", 953000000);
    } else {
        eim.setIntProperty("scale", room - 11);
    ...
    eim.setIntProperty("reward", 7);
    eim.setIntProperty("exp", 30);
    eim.setIntProperty("rank", 23);
    eim.setIntProperty("baseAch", 0);
    eim.setIntProperty("ach", 0);
    eim.setIntProperty("coin", 400);
    eim.setIntProperty("kaotic", 2);
    ...
    for (var i = 0; i < 6; i++) {
        ...
        map.parkKaoticSpawn(level, scale);
    }
