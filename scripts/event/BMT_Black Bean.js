...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "BMT_BlackBean_" + player.getName(), true);
...
function start(eim) {
    ...
    var mob = eim.getKaoticMonster(boss, eim.getValue("level"), eim.getValue("scale"), true, false, true, true, 999, true, true, false, true);
    eim.getMapInstance(eventMapId).spawnMonsterOnGroundBelow(mob, eim.newPoint(330, 750));
    var level = eim.getValue("level");
    var scale = eim.getValue("scale") - 5;
    for (var i = 1; i <= 20; i++) {
        var bean = eim.getKaoticMonster(9430027, level, scale, false, false, false);
        bean.setEventScript(false);
        map.spawnMonsterOnGroundBelow(bean, eim.newPoint(eim.getRandom(-400, 800), 500));
    }
