...
function setup(player, level, diff) {
    ...
    var hits = Math.floor(Math.pow(eim.getIntProperty("scale"), 1.25));
...
function waves(eim) {
    if (eim.getIntProperty("finish") < 1) {
        ...
        if (count < 80) {
            ...
            for (var i = 1; i <= 10; i++) {
                map.spawnMonsterOnGroundBelow(eim.getKaoticMonster(8860002, level, scale - 2, false, false, false), eim.newPoint(eim.getRandom(-2500, -300), -500));
            }
            for (var j = 1; j <= 5; j++) {
                map.spawnMonsterOnGroundBelow(eim.getKaoticMonster(8860001, level, scale - 1, false, false, false), eim.newPoint(eim.getRandom(-2500, -300), -500));
            }
        }
        eim.schedule("waves", eim.getRandom(5, 10) * 1000);
    }
}
