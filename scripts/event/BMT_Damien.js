...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "BMT_Damien_" + player.getName(), true);
...
function demon(eim) {
    if (eim.getValue("finished") == 0) {
        ...
        var random = eim.getRandom(8, 12);
        for (var i = 1; i <= random; i++) {
            var mobid = 9001041 + eim.getRandom(0, 1);
            map.spawnMonsterOnGroundBelow(eim.getMonsterNoAll(mobid, level, scale - 2), eim.newPoint(eim.getRandom(-600, 1250), 750));
        }
        eim.dropMessage(6, "[Damien] Come forth my greatest Demons!!!");
        eim.schedule("demon", eim.getRandom(30, 60) * 1000);
    }
}
