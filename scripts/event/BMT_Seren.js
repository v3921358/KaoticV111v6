...
function setup(player, level, scale) {
    var eim = em.newInstance(player, "BMT_Seren_" + player.getName(), true);
...
function start(eim) {
    var chance = eim.getRandom(1, 3);
    if (chance == 1) {
        var mob = eim.getKaoticMonster(8880600, eim.getValue("level"), eim.getValue("scale"), true, false, true, true, 999, true, true, false, false);
        eim.getMapInstance(eventMapId).spawnMonsterOnGroundBelow(mob, eim.newPoint(330, 750));
    }
    if (chance == 2) {
        var mob1 = eim.getKaoticMonster(8880601, eim.getValue("level"), eim.getValue("scale"), true, false, true, true, 999, true, true, false, false);
        eim.getMapInstance(eventMapId).spawnMonsterOnGroundBelow(mob1, eim.newPoint(330, 750));
    }
    if (chance == 3) {
        var mob2 = eim.getKaoticMonster(8880602, eim.getValue("level"), eim.getValue("scale"), true, false, true, true, 999, true, true, false, false);
        eim.getMapInstance(eventMapId).spawnMonsterOnGroundBelow(mob2, eim.newPoint(330, 750));
    }
...
function monsterKilled(mob, eim) {
    if (mob.getId() == 8880600 || mob.getId() == 8880601 || mob.getId() == 8880602) {
