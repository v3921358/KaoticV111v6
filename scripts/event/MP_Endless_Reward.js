...
function setup(player, level, scale, mode) {
    var eim = em.newInstance(player, "MP_Endless_Reward" + scale + "_" + player.getName(), true);
    ...
    eim.startEventTimer(eventTime * 60000 * level);
...
function playerEntry(eim, player) {
    ...
    player.dropMessage(6, "[Rogue] Every 5 Floors spanws a chest full of rewards");
...
function monsterKilled(mob, eim) {
    if (mob.getId() == 9601204) {
        var map = eim.getMapInstance(mob.getMap().getId());
        eim.showClear(map);
        eim.setValue("clear", 1);
        map.spawnReward(4, 100 * eim.getValue("tier"));
        eim.gainPartyStam(mob, 5 * eim.getValue("rate"));
        eim.gainPartyItem(4310506, eim.getValue("tier") * eim.getValue("rate"));
        var rand = eim.getRandom(1, 10);
        if (rand == 1) {
            eim.gainPartyItem(4420052, 1);
        }
    }
}
