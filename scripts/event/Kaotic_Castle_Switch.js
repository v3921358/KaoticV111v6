...
function setup(player, level, diff) {
    var eim = em.newInstance(player, "Kaotic_Castle_Switch" + player.getName(), true);
...
function begin(eim) {
    ...
    if (map != null) {
        map.setSpawnCap(999);
        map.SwitchSpawn(eim.getValue("level"), eim.getValue("scale"));
        map.mapEffect("Gstar/start");
        map.setStart(true);
    }
    eim.startEventTimer(1000 * 60 * 60);
    eim.dropMessage(6, "The door is now unlocked!");
    eim.dropMessage(6, "You have 1 hour to complete this quest.");
    eim.dropMessage(6, "Destory all the Switches and Monsters to clear the event.");
}
