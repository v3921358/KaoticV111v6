...
function setup(player, level) {
    var eim = em.newInstance(player, "Lucid_Farm_" + player.getName(), true);
...
function start(eim) {
    ...
    for (var i = 0; i < eim.getIntProperty("spawn"); i++) {
        map.spawnMonsterOnGroundBelow(eim.getMonster(8643016, level, 4), eim.newPoint(eim.getRandom(-500, 1800), -25));
    }
}
...
function waves(eim) {
    if (!eim.isDisposed()) {
        ...
        if (eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap() < eim.getIntProperty("spawn")) {
            var spawncount = 100 - eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap();
