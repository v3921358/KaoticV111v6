...
function setup(player) {
    var eim = em.newInstance(player, "TimeClock_" + player.getName(), true);
...
function start(eim) {
    ...
    var map = eim.getMapInstance(eventMapId);
    map.spawnMonsterOnGroundBelow(eim.getMonster(8910000), eim.newPoint(700, 400));
...
function bean(eim) {
    if (eim.getIntProperty("finished") < 1) {
        ...
        if (eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap() < 40) {
            eim.dropMessage(6, "[Von Bon] You cannot stop me!!!");
            for (var i = 1; i <= 10; i++) {
                map.spawnMonsterOnGround(eim.getMonsterNoAll(8910001, 300, 5), eim.newPoint(eim.getRandom(-300, 1500), 400));
            }
        }
