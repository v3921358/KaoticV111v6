...
function setup(player, scale) {
    var eim = em.newInstance(player, "EventBattle_" + player.getName(), true);
...
function start(eim) {
    ...
    eim.broadcastPlayerMsg(5, "Kill as many monsters as you can. Monsters Level: " + level + " - Tier " + eim.getIntProperty("scale"));
    ...
    if (!eim.isDisposed()) {
        ...
        if (map.getSpawnedMonstersOnMap() < 100) {
            ...
            for (var i = 0; i < spawncount; i++) {
                ...
                var mob = eim.getMonster(mobId, level, scale, false);
                map.spawnMonsterOnGroundBelow(mob, eim.newPoint(eim.getRandom(-500, 1000), -25));
            }
        }
