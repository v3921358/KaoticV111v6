...
var lobbyRange = [0, 0];
var weak = new Array(9480235,9480236,9480237,9480238);
...
function start(eim) {
    var nMob3 = eim.getKaoticMonster(boss, eim.getValue("level"), eim.getValue("scale"), true, false, true, true, 999, true, true, false, true);
    eim.spawnMonsterOnGroundBelow(eventMapId, nMob3, eim.newPoint(400, 10));
...
function randomboss(eim) {
    if (eim.getValue("finished") == 0) {
        if (eim.getValue("boss") < 4) {
            ...
            eim.dropMessage(6, "[Kahn] Come forth my servant and slay thee pests!");
            var mobid = weak[eim.getRandom(0, 3)];
            eim.getMapInstance(eventMapId).spawnMonsterOnGroundBelow(eim.getKaoticMonster(mobid, eim.getRandom(3000, 3800), eim.getRandom(70, 74), false, false, false), eim.newPoint(eim.getRandom(100, 600), 10));
        }
        eim.schedule("randomboss", 30000);
    }
}
...
function monsterKilled(mob, eim) {
    if (mob.getId() == 9480239) {
        eim.gainPartyItem(4310150, eim.getRandom(10, 25) * eim.getPlayerCount());
        eim.setValue("finished", 1);
        eim.victory(eim.getValue("exit"));
    }
    if (eim.getValue("finished") == 0) {
        if (mob.getId() == 9480235 || mob.getId() == 9480236 || mob.getId() == 9480237 || mob.getId() == 9480238) {
            eim.dropMessage(6, "[Kahn] How Dare you! I will get you for this!");
            eim.setValue("boss", eim.getValue("boss") - 1);
        }
    }
}
