    ...
    if (rm.getReactor().getState() == 5) {
        ...
        if (eim != null) {
            ...
            if (eim.getValue("orbs") <= 0 && !rm.getMap().getClear()) {
                rm.getMap().broadcastMapMsg("All the Demon Orbs has been destroyed. Map is Clear to progress.", 5120150);
                eim.getMapInstance(rm.getMap().getId()).showClear();
            } else {
                rm.getMap().broadcastMapMsg("Demon Orb has been destroyed. " + eim.getValue("orbs") + " Remaining", 5120150);
            }
        }
    }
