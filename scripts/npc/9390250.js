...
var job = "thieves";
var bitems = new Array(4009209, 4009210);
var bamount = new Array(2500, 2500);
var option = 0;
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getMapId() == 865020500) {
            option = 1;
            if (cm.getPlayer().getQuestLock(questid + 1) > 0) {
                cm.sendOk("Come see me after #b" + cm.secondsToString(cm.getPlayer().getQuestLock(questid + 1)) + "#k \r\n\      to repeat my quest again.\r\n\ Quest ID: " + (questid + 1));
            } else {
                var selStr = "";
                for (var i = 0; i < bitems.length; i++) {
                    var color = cm.haveItem(bitems[i], bamount[i]) ? "g" : "r";
                    selStr += "#i" + bitems[i] + "#  " + cm.getItemName(bitems[i]) + " (x#" + color + "" + bamount[i] + "#k)\r\n\ ";
                }
            ...
            if (cm.getPlayer().getQuestLock(questid) > 0) {
                ...
                var selStr = "";
                selStr += "#i" + items + "#  " + cm.getItemName(items) + " (x#r" + amount + "#k)\r\n\ ";
                var rewards = "#fUI/UIWindow.img/Quest/reward#\r\n\#fUI/UIWindow.img/Quest/basic#\r\n";
        ...
        if (option == 1) {
            if (cm.haveItem(bitems[0], bamount[0]) && cm.haveItem(bitems[1], bamount[1])) {
                cm.gainItem(bitems[0], -bamount[0]);
                cm.gainItem(bitems[1], -bamount[1]);
                cm.gainItem(reward, brewamount);
                cm.getPlayer().miniLevelUp();
                cm.getPlayer().setQuestLock(questid + 1, questtime);
                cm.sendOk("You have gained " + brewamount + "x #i" + reward + "#. Come back and see me in \r\n\#b" + cm.secondsToString(cm.getPlayer().getQuestLock(questid + 1)) + "#k.\r\n\ Quest ID: " + (questid + 1));
            } else {
                var selStr = "";
                for (var i = 0; i < bitems.length; i++) {
                    selStr += "#i" + bitems[i] + "#  " + cm.getItemName(bitems[i]) + " (x#r" + bamount[i] + "#k)\r\n\ ";
                }
