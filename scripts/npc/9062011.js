...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.haveItem(4420052, 1)) {
            if (!cm.getPlayer().isGroup()) {
                if (cm.getPlayer().achievementFinished(270)) {
                    if (cm.getPlayer().getTotalLevel() >= 200) {
                        var selStr = "Job Trials you will begin by selecting from random choice of jobs.\r\n";
                        selStr += "Completing the trails will either unlock the job or expand it's power.\r\n";
                        selStr += "This Trial Requires #b#i4420052# " + cm.getItemName(4420052) + "#k";
                        cm.sendYesNo("Are you Ready to begin your job trails?\r\n" + selStr);
                    } else {
                        cm.sendOkS("I must take on this Challenge being level 200+", 2);
                    }
                } else {
            ...
            cm.sendOkS("This trail requires #b#i4420052# " + cm.getItemName(4420052) + "#k", 2);
        }
    }
    if (status == 1) {
        if (cm.haveItem(4420052, 1)) {
            if (!cm.getPlayer().isGroup()) {
                var em = cm.getEventManager("MP_Endless_Job");
                if (em != null) {
                    var tier = 1 + cm.getPlayer().getVarZero(cm.getPlayer().getJob());
                    if (!em.startPlayerInstance(cm.getPlayer(), 1, tier, 50)) {
