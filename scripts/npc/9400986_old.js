    ...
    if (status == 0) {
        if (cm.haveItem(ticketId, 10)) {
            ...
            if (!equiplist.isEmpty()) {
                ...
                cm.sendSimple("Which NX equip would you like to STAT reset?\r\n\I only accept #i" + ticketId + "# (x10). This item's STATS are FULLY reseted, however Potentials are saved. Item Power is (" + (level) + ").\r\n\ " + selStr);
            } else {
                cm.sendOk("You currently do not have any Equips to reset.");

            }
        } else {
            cm.sendOk("You currently do not have Donation Points. Requires 10 Donation Points");

        }
    } else if (status == 1) {
        equip = equiplist.get(selection);
        cm.sendYesNo("Are you sure you want to reset this equip?");
    } else if (status == 2) {
        if (cm.haveItem(ticketId, 10)) {
            var power = (cm.getPlayer().getTotalLevel() / 10);
            cm.gainItem(ticketId, -10);
            cm.resetNXFixed(equip, power);
            cm.sendOk("Your equip's stats has been fully reset with power of " + (cm.convertNumber(power)));
