...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getVar("stumpy") < 8) {
            cm.sendOk("Hello child.");
        } else {
            if (cm.getPlayer().getVar("stumpy") == 8) {
                cm.getPlayer().setVar("stumpy", 9);
                cm.sendOk("#r[QUEST]#k Kobolds have been running in mass all through out the forest, Please take care of them some more.");
            } else if (cm.getPlayer().getVar("stumpy") == 9) {
                cm.sendOk("#r[QUEST]#k Go slay those Kobolds Slicers!!");
            } else if (cm.getPlayer().getVar("stumpy") == 10) {
                cm.getPlayer().setVar("stumpy", 11);
                cm.sendOk("#r[QUEST]#k Go deeper into the forest and find Papa Tom?");
            } else {
