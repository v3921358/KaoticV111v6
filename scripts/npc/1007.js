    ...
    if (status == 0) {
        if (!cm.getLockSlot()) {
            if (cm.getPlayer().getTotalLevel() >= level) {
                var selStr = "Welcome to #i" + cube + "# Machine ID: " + slot + ". Exp rate: #b" + cm.getSlotExp(slot) + "#k\r\n";
                ...
                cm.sendOk("This Slot machine is requires level: #r1000+#k.");
            }
        } else {
        ...
        option = selection;
        if (option == 1) {
            selStr = "Blue = Min Amount, Red = Combo Amount, Green = Jackpot amount:\r\n";
            selStr = "Higher Weights means more common that reward is.\r\n\r\n";
            var itemz = cm.getPlayer().getItemListFromSlot(slot);
            for (var i = 0; i < itemz.size(); i++) {
                selStr += star + "#b#z" + itemz.get(i).getId() + "##k\r\n";
                selStr += "          Amounts: #b" + itemz.get(i).getAmount() + "#k - #r" + itemz.get(i).getAmount2() + "#k - #g" + itemz.get(i).getAmount3() + "#k\r\n";
                selStr += "          Weight: #b" + itemz.get(i).getWeight() + "#k\r\n";
            }
