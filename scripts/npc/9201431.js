...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.getPlayer().getEventInstance() == null && cm.getPlayer().getMapId() == 610050400) {
                if (cm.getPlayer().isGroup()) {
                    if (cm.getPlayer().isLeader()) {
                        if (cm.getPlayer().achievementFinished(ach)) {
                            ...
                            cm.sendOk("You must complete the boss to the #rWEST#k you can challenge this one");
                        }
                    } else {
            ...
            if (cm.haveItem(items[0], amount[0]) && cm.haveItem(items[1], amount[1])) {
                if (cm.getPlayer().getStamina() >= stam) {
                    ...
                    if (em != null) {
                        if (em.getEligiblePartyAch(cm.getPlayer(), 3300, ach)) {
                            if (!em.startPlayerInstance(cm.getPlayer(), 3600, 36)) {
