...
function action(mode, type, selection) {
        ...
        if (amount == password) {
            if (cm.getPlayer().isGroup()) {
                if (cm.getPlayer().isLeader()) {
                    ...
                    text += "#L2# #rLevel: 6000 Tier: 150#k - Price: #b" + (2 * multi) + " " + itemName + "#k#l\r\n";
                    text += "#L3# #rLevel: 7000 Tier: 200#k - Price: #b" + (3 * multi) + " " + itemName + "#k#l\r\n";
                    text += "#L4# #rLevel: 8000 Tier: 250#k - Price: #b" + (4 * multi) + " " + itemName + "#k#l\r\n";
    ...
    if (status == 2) {
        var em = cm.getEventManager("boss_angel");
        if (em != null) {
            var level = 5000;
            var tier = 100;
            var cookies = multi;
            ...
            if (cm.haveItem(item, cookies)) {
                if (em.getEligiblePartyAch(cm.getPlayer(), level, ach, 1, 20) && em.getEligiblePartyAch(cm.getPlayer(), level, 273)) {
