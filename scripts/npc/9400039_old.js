...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        var text = "";
        text += "#L10# #i4310502# #rIncrease Overpower based on tier% #k#l\r\n";
        text += "#L11# #i4310502# #rIncrease Total Damage based on tier% #k#l\r\n";
        text += "#L12# #i4310502# #rIncrease Boss Damage based on tier% #k#l\r\n";
        text += "#L13# #i4310502# #rIncrease IED based on tier% #k#l\r\n";
        text += "#L14# #i4310502# #rIncrease Critial Damage based on tier% #k#l\r\n";
        text += "#L15# #i4310502# #rIncrease All Stats based on tier% #k#l\r\n";
        cm.sendSimple("Welcome to the Master NX Upgrading System. I can #rUpgrade#k any NX equip you have, but I charge a steep price\r\nWith the following Materials:\r\n" + text);
    } else if (status == 1) {
        stat = selection;
        ...
        if (!equiplist.isEmpty()) {
            ...
            for (var i = 0; i < equiplist.size(); i++) {
                ...
                if (curEquip != null && curEquip.isCash(curEquip.getItemId()) && curEquip.getUpgradeSlots() > 0) {
                    count += 1;
                    selStr += "#L" + i + "##i" + curEquip.getItemId() + "# " + cm.getItemName(curEquip.getItemId()) + " - #bUpgrades#k: " + curEquip.getUpgradeSlots() + "#l\r\n\ ";
                }
            }
            if (count > 0) {
        ...
        if (amount > 0 && amount <= equip.getUpgradeSlots()) {
            ...
            cm.sendYesNo("Do you confirm this selection?\r\n#rAny stats above cap will not be refunded.#b\r\n#rBonus Stats Cap: 999%.#k\r\n" + selStr);
        } else {
            cm.sendOk("You currently do not have any brains.");
        }
    } else if (status == 4) {
        if (equip != null) {
            ...
            if (stat == 10 || stat == 11 || stat == 12 || stat == 13 || stat == 14 || stat == 15) {
                if (cm.haveItem(scroll, finalcost)) {
