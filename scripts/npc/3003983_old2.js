...
var level = new Array(0, 0, 0, 0, 250, 500, 750, 1250, 2500);
var cost = new Array(25, 50, 100, 250, 500, 1000, 2500, 10000, 25000);
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var selStr = "Welcome to #rMESO BAGS#k Bonus Stat System. These stats are one time buy and are perm applied across the account.\r\nWhich Stat type would you like to explore?\r\n";
        ...
        for (var i = 0; i < ach.length; i++) {
            ...
            if (!cm.getPlayer().getAchievement(base + i)) {
                if (cm.getPlayer().getTotalLevel() >= level[i]) {
                    selStr += "#L" + i + "#" + star + "#b" + names[option] + " +" + amount + "" + txt + "#k (#rPrice: " + cost[i] + " MB#k)#l\r\n";
                } else {
        ...
        if (selection >= 210) {
            ...
            if (selection >= 200) {
                ...
                cm.sendYesNo("Do you want to confirm that you want to spend #r" + price + " Meso Bags#k on #b" + cm.getAchievementName(base) + "#k?");
            }
        }
    } else if (status == 3) {
        if (cm.haveItem(cube, price)) {
            ...
            cm.sendOk("You currently do not have enough Meso Bags for this bonus.");
        }
    }
}