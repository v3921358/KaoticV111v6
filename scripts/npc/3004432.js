...
function start() {
    var selStr = "Welcome to the #i" + gach + "# (#bSkin of Choice#k) Damage Skins Banner:\r\n";
    //selStr += ("   #L0##bWhats in this banner?#k#l\r\n");
    selStr += ("   #L0##bCash in Damage Tickets!#k#l\r\n");
    selStr += ("   #L1##bHow do I obtain this ticket?#k#l\r\n");
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (option == 1) {
            cm.sendOkS("You can get these tickets from @store inside the\r\n(#bDamage Skin Package#) #i2400003#.", 16);
        }
    } else if (status == 2) {
        if (option == 0) {
            if (!list.isEmpty()) {
                ...
                for (var i = 0; i < list.size(); i++) {
                    if (list.get(i).getTier() == selection) {
                        ...
                        if (!cm.getPlayer().hasSkin(skinId)) {
                            selStr += "#L" + skinId + "#";
                            selStr += "#fEffect/DamageSkin.img/" + skinId + "/NoCri1/7#";
                            selStr += "#l";
                            count++;
                        }
                    }
                ...
                if (count > 0) {
                    ...
                    cm.sendOkS("This banner does not have any " + tier[selection] + " Skins or you currently own all these skins.", 16);
                }
            } else {
        ...
        if (option == 0) {
            ...
            if (skin != null) {
                ...
                if (cm.haveItem(gach)) {
                    selSkin = skin.getId();
                    if (cm.getPlayer().hasSkin(selSkin)) {
                        cm.sendOkS("#rYou already own this skin.#k\r\n" + selStr, 16);
                    } else {
                        cm.sendYesNoS("#rWould you like to buy this skin?#k\r\n" + selStr, 16);
                    }
                } else {
                    cm.sendOkS(selStr, 16);
                }
            } else {
                cm.sendOkS("#rError with Skin - Please report.#k\r\n" + selStr, 16);
            }
        }
        if (option == 1) {
            status = 3;
