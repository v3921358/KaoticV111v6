...
function action(mode, type, selection) {
    ...
    if (status == 0 && mode == 1) {
        cm.sendSimple("Welcome to the Label Ring Gachapon\r\n#rThese Rings are purely cosmetic#k\r\n#rEach ring costs 10 " + cm.getItemName(ticketId) + "s#k\r\n#L4#Cash in for a Chat Label Gachapon?#l");
    } else if (status == 1) {
        if (selection == 4) {
            rewards = cm.getAllRewards(9110071);
            if (!rewards.isEmpty()) {
        ...
        if (cm.haveItem(ticketId, 10)) {
            ...
            cm.sendOk("You don't have 10 #i" + ticketId + "# " + cm.getItemName(ticketId) + ".");
        }
    }
}