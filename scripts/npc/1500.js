...
function action(mode, type, selection) {
    ...
    if (status == 0) {//-------------------------------------------------------------------------------------------
        ...
        text += "#L80#" + star + "#rGoto My Laboratory#k#l\r\n";
        ...
        if (option == 10) {
            ...
            txt += "#L64##fUI/Custom.img/shared/icon/9# #bChange me Pal's Element#k#l\r\n";
        ...
        if (option == 64) {
            cm.sendYesNo("Are you sure you want to use #i4202049#\r\nto change your Pal's #bElement#k?\r\n#rThis feature cannot be undone#k\r\n#rElement will be chosen at random.#k");
        }
        if (option == 61) {
            var skill = cm.getPalSkill(pal.skill);
        ...
        if (option == 62) {
            upgrade = 4201001;
            cm.sendGetText("How many #i" + upgrade + "# do you want to apply to #r" + pal.getName() + "#k.\r\nThese souls are used to increase a Pals Max HP.\r\n#rEach Upgrade costs 10 souls, How many upgrades you want?#k\r\nCurrently have #b" + cm.convertNumber(cm.getPlayer().countAllItem(upgrade)) + "#k");
        }
            ...
            if (recv != null) {
                if (recv.canHoldPal() && cm.getPlayer().getMap() == recv.getMap()) {
                    if (cm.haveItem(4202068, 1)) {
                        ...
                        cm.sendOk("You do not have the #z4202068# to send this pal.");
                    }
                } else {
        ...
        if (option == 64) {
            if (cm.haveItem(4202049, 1)) {
                cm.gainItem(4202049, -1);
                var ele = cm.random(0, 9);
                pal.setElement(ele);
                pal.save();
                cm.sendOk("Your Pal's Element has changed to #fUI/Custom.img/shared/element/" + ele + "#.");
            } else {
                cm.sendOk("You do not have enough #i4202027#.");
            }
        }
        ...
        if (option == 62) {
            uAmount = cm.getNumber();
            if (cm.haveItem(upgrade, uAmount * 10)) {
                cm.sendYesNo("Are you sure you want to apply #b" + (uAmount * 10) + "#k #i" + upgrade + "# to increase your pal's Max Hp by #r" + uAmount + "#k.");
            } else {
        ...
        if (option == 62) {
            if (cm.haveItem(upgrade, uAmount * 10)) {
                cm.gainItem(upgrade, -(uAmount * 10));
                pal.addStat(0, uAmount);
                pal.save();
                cm.sendOk(pal.getName() + " has gained #b+" + uAmount + " HP#k.");
            } else {
                cm.sendOk("You cannot apply this many upgrades to this pal.");
            }
        }
