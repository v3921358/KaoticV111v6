    ...
    if (cm.getPlayer().achievementFinished(408)) {
        if (cm.getPlayer().isGroup()) {
            if (cm.getPlayer().isLeader()) {
                ...
                var text = "You think you can tear my ass up?? lets go, and you better have brought my fucking #b#i4036657# " + cm.getItemName(4036657) + "s#k (#b" + cm.convertNumber(cm.getPlayer().countAllItem(4036657)) + "#k)\r\n";
                var group = cm.getPlayer().getGroupSize();
                //text += "#L1# #rCash in #i" + orb + "#" + cm.getItemName(orb) + " for Fel Power!#k#l\r\n\r\n";
                text += "#L1# #rLevel: 5000 Tier: 90#k - Price: #b1000 #i4036657##k#l\r\n";
                text += "#L2# #rLevel: 5000 Tier: 95#k - Price: #b5000 #i4036657##k#l\r\n";
                text += "#L3# #rLevel: 5000 Tier: 100#k - Price: #b25000 #i4036657##k#l\r\n";
                cm.sendSimple(text);
            } else {
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        var em = cm.getEventManager("boss_ice");
        if (em != null) {
            var level = 5000;
            var tier = 90;
            var cookies = 1000;
            ...
            if (cm.haveItem(4036657, cookies)) {
                if (em.getEligiblePartyAch(cm.getPlayer(), level, 408, 1, 6)) {
                    if (!em.startPlayerInstance(cm.getPlayer(), level, tier)) {
                        cm.sendOk("Someone is already attempting the PQ or your instance is currently being reseted. Try again in few seconds.");
                    } else {
                        cm.gainItem(4036657, -cookies);
                    }
                } else {
                ...
                cm.sendOk("Where is my #r" + cookies + "#k #b#i4036657# " + cm.getItemName(4036657) + "s#k????");
            }
        } else {
