...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getMapId() == 865030201) {
            option = 1;
            if (cm.getPlayer().getQuestLock(bquestid) > 0) {
                ...
                var selStr = "";
                var color = cm.haveItem(bitems, amount) ? "g" : "r";
                selStr += "#i" + bitems + "#  " + cm.getItemName(bitems) + " (x#" + color + "" + amount + "#k)\r\n\ ";
        ...
        if (option == 1) {
            if (cm.haveItem(bitems, amount)) {
                cm.gainItem(bitems, -amount);
