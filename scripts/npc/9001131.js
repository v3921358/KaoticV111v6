...
var amount = 0;
function start() {
    var total = cm.convertNumber(cm.getServerVar("xmas"));
    var rewards = "Server has saved #r" + total + "#k #bPresents#k (#bEvent!#k)\r\n";
    rewards += "You have saved #r" + cm.getPlayer().getVarZero("exmas") + "#k #bPresents#k (#bEvent!#k)\r\n\r\n";
        ...
        if (amount > 0 && amount <= 100000) {
            if (cm.haveItem(etc, 25 * amount)) {
                if (cm.canHold(key)) {
                    ...
                    cm.addServerVar("xmas", tot);
                    cm.getPlayer().addVar("xmas", tot);
