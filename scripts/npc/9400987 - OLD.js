...
function action(mode, type, selection) {
    ...
    if (status == 0 && mode == 1) {
        cm.sendSimple("Welcome to the Seasonal Gachapon. How may I help you?\r\n\r\n#L5#Cash in 10 #i4310502#?#l\r\n#L4#What are the rewards in this DP Gachapon?#l");
    } else if (status == 1) {
        if (selection == 4) {
            var rewards = cm.getAllRewards(gachid);
            if (!rewards.isEmpty()) {
        ...
        if (cm.haveItem(ticketId, amount) && amount > 0) {
            ...
            var rewards = cm.getRewardsByPower3(gachid, cm.getPlayer(), 750000, 30);
            if (rewards == null) {
