...
function action(mode, type, selection) {
        ...
        if (amount == password) {
            if (cm.getPlayer().isGroup()) {
                if (cm.getPlayer().isLeader()) {
                    var text = "You better have brought my #b" + itemName + " " + cm.getItemName(item) + "s#k\r\nCurrent Amount: (#b" + cm.convertNumber(cm.getPlayer().countAllItem(item)) + "#k)\r\n#rParty Range 4-40 Members#k\r\n";
                    //text += "#L1# #rCash in #i" + orb + "#" + cm.getItemName(orb) + " for Fel Power!#k#l\r\n\r\n";
                    text += "#L1# #rLevel: 9000 Tier: 200#k - Price: #b" + (1) + "#k#l\r\n";
                    text += "#L2# #rLevel: 9100 Tier: 250#k - Price: #b" + (5) + "#k#l\r\n";
                    text += "#L3# #rLevel: 9200 Tier: 300#k - Price: #b" + (10) + "#k#l\r\n";
                    text += "#L4# #rLevel: 9300 Tier: 400#k - Price: #b" + (25) + "#k#l\r\n";
                    text += "#L5# #rLevel: 9400 Tier: 500#k - Price: #b" + (50) + "#k#l\r\n";
                    text += "#L6# #rLevel: 9500 Tier: 750#k - Price: #b" + (100) + "#k#l\r\n";
                    text += "#L7# #rLevel: 9999 Tier: 999#k - Price: #b" + (250) + "#k#l\r\n";
    ...
    if (status == 2) {
        var em = cm.getEventManager("boss_limbo");
        if (em != null) {
            var level = 9000;
            var tier = 200;
            ...
            if (cm.haveItem(item, cookies)) {
                if (em.getEligiblePartyAchReborn(cm.getPlayer(), level, ach, 4, 40, 0) && em.getEligiblePartyAch(cm.getPlayer(), level, 273)) {
