...
function action(mode, type, selection) {
    ...
    if (status == 0 && mode == 1) {
        ...
        text += "#L5#Redeem #b" + cm.getItemName(5222138) + "#k#l\r\n";
        ...
        if (option == 5) {
            ...
            if (cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.CASH).getNumFreeSlot() >= 1) {
                if (cm.haveItem(ticketId, 1)) {
                    cm.sendYesNo("Hello#b #h ##k, Would you like to exchange #b" + cm.getItemName(5222138) + "#k for a random Pet?\r\n#kEach Pet is Cosmetic only and will last forever.");
                } else {
                    cm.sendOk("You don't have enough Pet Boxes.");
                }
            } else {
