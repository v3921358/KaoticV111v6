...
var amount = new Array(1000, 100);
var Cantidad;
var boss = "Butcher";
var ach = 402;
var instance = "BGC_Butcher";
var option = 0;
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.getPlayer().getEventInstance() == null && cm.getPlayer().getMapId() == 610050700) {
                if (cm.getPlayer().isGroup()) {
                    if (cm.getPlayer().isLeader()) {
                        if (cm.getPlayer().achievementFinished(ach)) {
                            ...
                            cm.sendOk("You must complete the boss to the #rNORTH#k you can challenge this one");
                        }
                    } else {
                ...
                if (cm.getPlayer().getMapId() == 610052100) {
                    if (cm.getPlayer().achievementFinished(405)) {
                        cm.sendYesNo("Do you want travel over to the #rHIDDEN MAP#k?");
                    } else {
                        cm.sendOk("You must defeat Khan to enter here.");
                    }
            ...
            if (option == 1) {
                if (cm.haveItem(items[0], amount[0]) && cm.haveItem(items[1], amount[1])) {
                    ...
                    if (cm.getPlayer().getStamina() >= stam) {
                        ...
                        if (em != null) {
                            if (em.getEligiblePartyAch(cm.getPlayer(), 3500, ach)) {
                                if (!em.startPlayerInstance(cm.getPlayer(), 3800, 38)) {
