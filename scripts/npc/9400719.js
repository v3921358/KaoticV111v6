    ...
    if (status == 0) {
        if (cm.getPlayer().isGroup()) {
            if (cm.getPlayer().isLeader()) {
                cm.sendSimple("Which ACPQ Challenge do want to challenge?\r\n#L1##v3994115##l#L2##v3994116##l#L3##v3994117##l#L4##v3994118##l#L5##l\r\n\r\n#rEach Mode costs (10-15-20-25) Stamina#k.\r\n#rEach Mode requires (2-3-4-4) players#k.\r\n#rEach Mode requires Level (100-150-200-250) players#k.");
            } else {
        ...
        var em = cm.getEventManager("APQ");
        if (em != null) {
            var stamina = 5 + (5 * selection);
            if (cm.getPlayer().getStamina() >= stamina) {
                if (em.getEligibleParty(cm.getPlayer(), level[selection], 1, 6)) {
