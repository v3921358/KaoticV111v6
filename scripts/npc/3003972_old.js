...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().isGM()) {
            cm.sendYesNo("Need a ride over to World's Sorrow?");
        } else {
            if (cm.getPlayer().getMapId() == 450009050) {
                if (cm.getPlayer().getTotalLevel() >= level) {
                    if (cm.getPlayer().getAchievement(69)) {
                        ...
                        cm.sendOk("You must have defeated Evil Hilla before you can venture to World's Sorrow.");
                    }
                } else {
