...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (!equiplist.isEmpty()) {
            ...
            for (var i = 0; i < equiplist.size(); i++) {
                ...
                if (curEquip != null && curEquip.getPower() < 99) {
                    count += 1;
                    var price = parseInt(1 + Math.floor(curEquip.getPower() * 0.1));
        ...
        cost = parseInt(1 + Math.floor(equip.getPower() * 0.1));
        if (cm.haveItem(cube, cost)) {
            cm.sendYesNoS("Would you like to upgrade\r\n#i" + equip.getItemId() + "# " + cm.getItemName(equip.getItemId()) + " to #bTier " + (equip.getPower() + 1) + "#k? The Price is " + cost + " #i4310505#", 16);
        } else {
