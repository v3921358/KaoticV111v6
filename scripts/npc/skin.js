...
function start() {
    ...
    selStr += "Collected Skins: #b" + cm.getPlayer().getDamageSkins().size() + "#k/#r588#k\r\n";
    ...
    if (skini < 9000) {
        ...
        var lvl = cm.getPlayer().getSkinLevel(skind.getId());
        selStr += star + "Current Level: #b" + lvl + "#k\r\n";

        if (skind.getAmount() > 0) {
            selStr += star + "Current Bonus Stats: #b+" + (skind.getAmount() * lvl) + "% " + stat[skind.getStat()] + "#k\r\n";
        }
        if (skind.getAmount2() > 0) {
            selStr += star + "Current Bonus Stats: #b+" + (skind.getAmount2() * lvl) + "% " + stat[skind.getStat2()] + "#k\r\n";
        }
        if (skind.getAmount3() > 0) {
            selStr += star + "Current Bonus Stats: #b+" + (skind.getAmount3() * lvl) + "% " + stat[skind.getStat3()] + "#k\r\n";
        }
        /*
    ...
    if (status == 1) {
        ...
        for (var i = 0; i < list.size(); i++) {
            ...
            if (skinz.getTier() == selection) {
                ...
                selStr += "ID: #b" + skinId + "#k - Current Level: #b" + lvl + "#k\r\n";
                if (option == 2) {
                    if (skinz.getAmount() > 0) {
                        selStr += "                " + star + "Current Bonus Stats: #b+" + (skinz.getAmount() * lvl) + "% " + stat[skinz.getStat()] + "#k\r\n";
                    }
                    if (skinz.getAmount2() > 0) {
                        selStr += "                " + star + "Current Bonus Stats: #b+" + (skinz.getAmount2() * lvl) + "% " + stat[skinz.getStat2()] + "#k\r\n";
                    }
                    if (skinz.getAmount3() > 0) {
                        selStr += "                " + star + "Current Bonus Stats: #b+" + (skinz.getAmount3() * lvl) + "% " + stat[skinz.getStat3()] + "#k\r\n";
                    }
                }
            }
        }
        ...
        selStr += star + "Current Level: #b" + lvl + "#k\r\n";
        if (skin.getAmount() > 0) {
            selStr += star + "Current Bonus Stats: #b+" + (skin.getAmount() * lvl) + "% " + stat[skin.getStat()] + "#k\r\n";
        }
        if (skin.getAmount2() > 0) {
            selStr += star + "Current Bonus Stats: #b+" + (skin.getAmount2() * lvl) + "% " + stat[skin.getStat2()] + "#k\r\n";
        }
        if (skin.getAmount3() > 0) {
            selStr += star + "Current Bonus Stats: #b+" + (skin.getAmount3() * lvl) + "% " + stat[skin.getStat3()] + "#k\r\n";
        }
        /*
