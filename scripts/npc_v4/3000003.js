    ...
    if (cm.getPlayer().getTotalLevel() >= 999) {
        if (cm.getPlayer().isGroup()) {
            if (cm.getPlayer().isLeader()) {
                var price = cost * cm.getPlayer().getGroupSize();
                cm.sendYesNo("Ready to challenge Mini Pantheon Raid?\r\n#rGroup Size Min: 1 - Max:4#k\r\nPrice: #r" + price + "#k #b#i" + item + "# " + cm.getItemName(item) + "#k");
            } else {
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        cost = cost * cm.getPlayer().getGroupSize();
        if (cm.haveItem(item, cost)) {
            var em = cm.getEventManager("Blake_JR");
            if (em != null) {
                if (em.getEligiblePartyAch(cm.getPlayer(), 999, 66, 1, 4)) {
