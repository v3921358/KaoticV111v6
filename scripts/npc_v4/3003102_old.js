...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getTotalLevel() >= 1000) {
            ...
            if (!equiplist.isEmpty()) {
                ...
                for (var i = 0; i < equiplist.size(); i++) {
                    ...
                    if (curEquip != null && curEquip.getPower() < 50) {
                        count += 1;
            ...
            cm.sendOkS("I must be level 1000 of higher to upgrade my equips.", 16);
        }
    } else if (status == 1) {
        if (equip == null) {
        ...
        if (cm.haveItem(cube, cost)) {
            cm.sendYesNoS("Would you like to upgrade\r\n#i" + equip.getItemId() + "# " + cm.getItemName(equip.getItemId()) + " to #bTier " + (equip.getPower() + 1) + "#k? The Price is " + cost + " #i4310150#", 16);
        } else {
        ...
        if (cm.haveItem(cube, cost)) {
            if (equip != null) {
                ...
                cost = equip.getPower() * equip.getPower();
                if (cm.haveItem(cube, cost) && equip.getPower() < 50) {
                    cm.sendYesNoS(text + "Would you like to upgrade to #bTier " + (equip.getPower() + 1) + "#k?\r\nNext Tier price is " + cost + " #i4310150#.", 16);
                } else {
                    cm.sendOkS(text, 16);
                }
            ...
            cm.sendOkS("You currently do not have enough #i4310150#. Requires " + cube + "#s", 16);
