...
var bitems = new Array(4032906, 4034180, 4034181, 4034182, 4034183);
var bamount = new Array(2500, 1000, 1000, 1000, 1000);
var option = 0;
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
                        ...
                        if (cm.getPlayer().isLeader()) {
                            if (cm.getPlayer().achievementFinished(404)) {
                                ...
                                for (var i = 0; i < bitems.length; i++) {
                                    selStr += "#i" + bitems[i] + "#  " + cm.getItemName(bitems[i]) + " (x" + bamount[i] + ") (#b" + cm.convertNumber(cm.getPlayer().countAllItem(bitems[i])) + "#k)\r\n\ ";
                                }
                                cm.sendYesNo("I need the following materials to battle Kahn: \r\n\r\n\ " + selStr);
                            } else {
            ...
            if (option == 2) {
                if (cm.haveItem(bitems[0], bamount[0]) && cm.haveItem(bitems[1], bamount[1]) && cm.haveItem(bitems[2], bamount[2]) && cm.haveItem(bitems[3], bamount[3]) && cm.haveItem(bitems[4], bamount[4])) {
                    em = cm.getEventManager(instance);
                    if (em != null) {
                        if (em.getEligiblePartyAch(cm.getPlayer(), 3600, 185)) {
                            if (!em.startPlayerInstance(cm.getPlayer(), 4000, 40)) {
                                cm.sendOkS("Someone is already attempting the PQ or your instance is currently being reseted. Try again in few seconds.", 16);
                            } else {
                                cm.gainItem(bitems[0], -bamount[0]);
                                cm.gainItem(bitems[1], -bamount[1]);
                                cm.gainItem(bitems[2], -bamount[2]);
                                cm.gainItem(bitems[3], -bamount[3]);
                                cm.gainItem(bitems[4], -bamount[4]);
                            }
                        } else {
