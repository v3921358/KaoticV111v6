...
function start() {
    ...
    text += "#L4##bInfinity Shop#k#l\r\n";
    text += "#L3##rExchange Packages for Rewards#b#l\r\n";
    if (cm.getPlayer().getAccVara("KP_BOOST") < 1000) {
        text += "#L5##gBuy KP Rate for 100 DP#k (Max 1000 KPR)#l\r\n";
    }
    text += "#L8##rConvert Donation Point to 25 Spell Trace#b#l\r\n";
    if (cm.getDonationRate() >= 1) {
        text += "#L7##rAdd to Chaos Hour#b (#bInfinite Point#k)#l\r\n";
    }
    text += "#L10##rExpand Etc% Cap#b#l\r\n";
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (option == 11) {
            cm.sendOk(qr + "\r\n\r\n\r\n#rScan QR below to donate ETH#k\r\nContact Staff in Discord once you Crypto has sent with the transaction log or receipt.");
        }
        if (option == 2) {
            var text = "";
            if (bonus > 1) {
        ...
        if (option == 7) {
            ...
            if (cm.haveItem(4310505, amount)) {
                cm.sendYesNo("Do you want to confirm that you wish to spend\r\n#r" + amount + " #i4310505# " + cm.getItemName(4310505) + "#k on #bChaos Hour Bonus Rates#k?");
            } else {
            ...
            if (codeinfo != null) {
                if (cm.canHold(codeinfo.itemId(), codeinfo.amount())) {
                    if (cm.tryRedeemCode(codeinfo)) {
                        ...
                        if (codeinfo.itemId() == 4310504) {
                            var a = codeinfo.amount() * bonus * 5;
                            cm.addKaoticPowerTime(a);
                        }
