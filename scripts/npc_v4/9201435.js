    ...
    if (status == 0) {
        if (cm.getPlayer().getMapId() != 610052100) {
            ...
            if (cm.getPlayer().getQuestLock(questid) > 0) {
                ...
                var selStr = "";
                selStr += "#i" + items + "#  " + cm.getItemName(items) + " (x#r" + amount + "#k) (#b" + cm.convertNumber(cm.getPlayer().countAllItem(items)) + "#k)\r\n\ ";
        ...
        if (cm.haveItem(items, amount)) {
            ...
            cm.sendOk("You currently do not have enough #i4033337#  " + cm.getItemName(items) + " for me to give you #i" + reward + "#.");
        }
        cm.dispose();
    } else {
        cm.dispose();
