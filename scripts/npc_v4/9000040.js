    ...
    if (mode == -1) {
        ...
        if (status == 1) {
            ...
            if (equip != null && equip != leaf) {
                amount = cm.getPlayer().getOverflowAmount(equip);
                cm.sendYesNo("Do you want to convert #rAll#k #i" + equip + "# to Unleased Coins?\r\n\You currently have #b" + cm.convertNumber(amount) + "#k.");
            } else {
                cm.sendOk("You cannot convert this item, #rDumbAss#k.");
            }
        } else if (status == 3) {
            if (equip != null && equip != leaf) {
                if (amount > 0 && Math.floor(cm.getPlayer().getOverflowAmount(equip)) >= amount) {
                    cm.sendYesNo("Do you want to confirm you want to exchange:\r\n #b" + amount + " #i" + equip + "##k into #r#i" + leaf + "##k?");
                } else {
                    cm.sendOk("You do not have #r" + amount + "#k #i" + equip + "#.");
