...
function action(mode, type, selection) {
    ...
    if (status == 0 && mode == 1) {
        4310100
        var text = "#L4##bWhat are the rewards in this Armor Gachapon?#k#l\r\n\r\n";
        ...
        cm.sendSimple("Welcome to the Commerci Armor Gachapon.\r\nHow may I help you?\r\n#rYour Current Commi-Power: "+100+"#k\r\n" + text);
    } else if (status == 1) {
        if (selection == 4) {
            var rewards = cm.getAllRewards(9110020);
            if (!rewards.isEmpty()) {
        ...
        if (selection >= 5) {
            ...
            if (cm.haveItem(ticketId, cost)) {
                option = 1;
                cm.sendYesNo("Hello#b #h ##k, Would you like to exchange #r" + cost + "#k - #i4310100# for a random Tier: #b" + scale + "#k Sweetwater Armor?");
            } else {
                option = 2;
        ...
        if (cm.haveItem(ticketId, cost)) {
            ...
            var rewards = cm.getRewardsByPower2(9110020, cm.getPlayer(), 100, scale);
            if (rewards == null) {
