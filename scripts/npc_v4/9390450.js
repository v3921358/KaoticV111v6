...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getVar("stumpy") < 11) {
            cm.sendOk("Hello child.");
        } else {
            if (cm.getPlayer().getVar("stumpy") == 11) {
                cm.getPlayer().setVar("stumpy", 12);
                cm.sendOk("#r[QUEST]#k Kobolds have been running in mass all through out the forest, Please take care of them some more.");
            } else if (cm.getPlayer().getVar("stumpy") == 12) {
                cm.sendOk("#r[QUEST]#k Go slay those Kobold Pokers!!");
            } else if (cm.getPlayer().getVar("stumpy") == 13) {
                cm.getPlayer().setVar("stumpy", 14);
                cm.sendOk("#r[QUEST]#k Go deeper into the forest and find Bluffing Tom?");
            } else {
