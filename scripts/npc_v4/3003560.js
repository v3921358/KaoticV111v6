    ...
    if (cm.getPlayer().isGroup()) {
        if (cm.getPlayer().isLeader()) {
            if (cm.getPlayer().getTotalLevel() >= level) {
                if (cm.haveItem(4001893)) {
                    cm.sendYesNo("Do you have the determination to challenge Will?");
                } else {
                    cm.sendOk("I want nothing to do with you if you do not have #i" + 4001893 + "#.");
                }
            } else {
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (em != null) {
            if (em.getEligiblePartyAch(cm.getPlayer(), level, 64)) {
