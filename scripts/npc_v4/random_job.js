...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            var L = 10 * (1 + cm.getPlayer().getVarZero("job"));
            price = Math.floor(Math.pow(L, 2.5));
            cm.sendYesNo("Would you like to change jobs for the #bsmall price#k: \r\n#i4310500# (#r" + price + "x#k)?\r\n#rThis price rapidly increases per change, so be sure of your decision.#k");
        } else if (status == 1) {
            if (cm.getPlayer().getTotalLevel() >= 1000) {
                if (cm.haveItem(4310500, price)) {
                    ...
                    text += "#L900##b Daddy Master #k#l\r\n";
                    text += "#L2312##b Jett Baby #k#l\r\n";
                    text += "#L3512##b Burster Mommy #k#l\r\n";
                    text += "#L2112##b Aran-itain #k#l\r\n";
                    cm.sendSimple("Select which job you would like to become!\r\n" + text);
                } else {
                    cm.sendOk("Seems like you dont have enough Meso Bags for the tranformation.!");
                }
            } else {
            ...
            job = selection;
            cm.sendYesNo("Are you sure you want to change jobs?\r\n#rAll skills set to macro and keybinds will be reset#k.");
        } else if (status == 3) {
            if (cm.haveItem(4310500, price)) {
                cm.gainItem(4310500, -price);
                cm.getPlayer().switchJob(job);
                cm.getPlayer().addVar("job", 1);
                cm.sendOk("You have successfully changed jobs!");
            } else {
                cm.sendOk("Seems like you dont have enough Meso Bags for the tranformation.!");
            }
        } else {
