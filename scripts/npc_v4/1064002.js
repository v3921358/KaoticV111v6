            ...
            if (cm.getPlayer().isLeader()) {
                if (cm.getPlayer().getTotalLevel() >= level) {
                    ...
                    if (cm.getPlayer().getTotalLevel() >= 750) {
                        text += "#L1# #rReq. #k#bLvl. 750 Kaotic Tier: 30#k (Keys: 1)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 1000) {
                        text += "#L2# #rReq. #k#bLvl. 1000 Kaotic Tier: 40#k (Keys: 2)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 1500) {
                        text += "#L3# #rReq. #k#bLvl. 1500 Kaotic Tier: 50#k (Keys: 5)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 2000) {
                        text += "#L4# #rReq. #k#bLvl. 2000 Kaotic Tier: 60#k (Keys: 25)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 2500) {
                        text += "#L5# #rReq. #k#bLvl. 2500 Kaotic Tier: 70#k (Keys: 100)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 5000) {
                        text += "#L6# #rReq. #k#bLvl. 5000 Kaotic Tier: 80#k (Keys: 500)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 7500) {
                        text += "#L7# #rReq. #k#bLvl. 7500 Kaotic Tier: 90#k (Keys: 1000)#l\r\n";
                    }
                    if (cm.getPlayer().getTotalLevel() >= 9999) {
                        text += "#L8# #rReq. #k#bLvl. 9999 Kaotic Tier: 99#k (Keys: 10000)#l\r\n";
                    }
