        ...
        if (cm.getPlayer().isGroup()) {
            if (cm.getPlayer().isLeader()) {
                if (cm.getPlayer().getTotalLevel() >= level) {
                    if (cm.haveItem(4033321, 1000)) {
                        cm.sendYesNo("Are you ready to take down evil slime boss thats causing all this trouble here? I will need 1000 #i4033321# to drawn it in.");
                    } else {
                        cm.sendOk("You must have 1000 #i4033321# in your inventory to enter here.");
                    }
                } else {
            ...
            if (cm.getPlayer().isLeader()) {
                if (cm.getPlayer().getTotalLevel() >= level) {
                    if (cm.haveItem(4021021, 1)) {
                        cm.sendYesNo("Are you ready to take down Commander Will thats causing all this trouble here? I will need #i4021021# to drawn it in.");
                    } else {
                        cm.sendOk("You must have #i4021021# in your inventory to enter here.");
                    }
                } else {
                    cm.sendOk("The leader must be level " + level + " or higher to complete this event.");
                }
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        if (option == 1) {
            ...
            if (em != null) {
                if (em.getEligiblePartyAch(cm.getPlayer(), level, 180)) {
                    if (!em.startPlayerInstance(cm.getPlayer())) {
                        cm.sendOk("Someone is already attempting the PQ or your instance is currently being reseted. Try again in few seconds.");
                    } else {
