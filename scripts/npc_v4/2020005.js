...
function action(mode, type, selection) {
	...
	if (status == 2) {
	    cm.sendOk("I see. Understand that I have many different items here. Take a look around. I'm only selling these items to you, so I won't be ripping you off in any way shape or form.");
	    return;
	}
	status--;
    }
	...
	if(cm.getMeso() < totalcost || !cm.canHold(item[selected])) {
	    cm.sendOk("Are you sure you have enough mesos? Please check and see if your etc. or use inventory is full, or if you have at least #r"+totalcost+"#k mesos.");
	                
	}
	cm.gainMeso(-totalcost);
	cm.gainItem(item[selected], amount);
	cm.sendOk("Thank you. If you ever find yourself needing items down the road, make sure to drop by here. I may have gotten old over the years, but I can still make magic items with ease.");
    }
}