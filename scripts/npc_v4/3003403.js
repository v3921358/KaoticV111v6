...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getQuestLock(questid) > 0) {
            ...
            cm.sendGetText("#rYour Jail time has expired your free to go, enter code to proceed.\r\n#rDo not mess up the code else its more time in the jail house!#k\r\n\r\n\     " + cm.getDamageSkinNumber(number1, number2, number3, number4) + "");
        }
    } else if (status == 1) {
        amount = cm.getNumber();
        if (amount == password) {
            cm.sendYesNo("Your free to go....");
        } else {
            cm.getPlayer().setQuestLock(questid, 3600);
            cm.sendOk("Wrong password mother fka. Jail time has been extended by 1 hour.");
        }
    } else if (status == 2) {
        cm.getPlayer().changeMapbyId(100000000);
        cm.dispose();
    } else {
        cm.dispose();
    }
