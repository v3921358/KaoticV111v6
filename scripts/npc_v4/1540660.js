...
var reward = 4001897;
var rewamount = 1;
var items = new Array(4001894, 4033800, 4033801, 4033802);
var amount = new Array(100, 100000, 100000, 100000);
var Cantidad;
function start() {
    status = -1;
    action(1, 0, 0);
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            for (var i = 0; i < items.length; i++) {
                selStr += "#i" + items[i] + "#  #b" + cm.getItemName(items[i]) + "#k (x#r" + amount[i] + "#k) (#b" + cm.convertNumber(cm.getPlayer().countAllItem(items[i])) + "#k)\r\n\ ";
            }
            ...
            if (cm.haveItem(items[0], amount[0]*Cantidad) && cm.haveItem(items[1], amount[1]*Cantidad) && cm.haveItem(items[2], amount[2]*Cantidad) && cm.haveItem(items[3], amount[3]*Cantidad)) {
                cm.gainItem(items[0], -amount[0]*Cantidad);
                cm.gainItem(items[1], -amount[1]*Cantidad);
                cm.gainItem(items[2], -amount[2]*Cantidad);
                cm.gainItem(items[3], -amount[3]*Cantidad);
                cm.gainItem(reward, rewamount*Cantidad);
                cm.sendOk("You have gained #i" + reward + "#x"+Cantidad+", use this to fight my daddy, Darknell. Sorry Daddy!!");
            } else {
