...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (cm.haveItem(item, amount) && cm.haveItem(mbag, mAmount)) {
            ...
            cm.gainItem(reward, 25000);
            cm.getPlayer().setVaraLock("Moral_" + cm.getNpc(), 3600 * 4);
            cm.getPlayer().setAccVar("Morale", cm.setMaxValue(cm.getPlayer().getAccVara("Morale") + 25000, 9999999));
